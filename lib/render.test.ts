...
// Grouping (GROUP_JS) is off by default now; these tests exercise the
// bundling path, so opt in explicitly. render.ts reads the env var lazily
// per call, so staging it here is safe regardless of import order.
const _restore_group_js = process.env.GROUP_JS;
process.env.GROUP_JS = "true";
...
describe("move_styles_and_scripts_to_head", () => {
	...
	describe("initialize_render / get_render", () => {
		test("initialize_render sets up render function", async () => {
			...
			const { get_render } = await import("./render");
	...
	describe("issue reporter injection", () => {
		test("injects the issue reporter into Reeman pages", async () => {
			const engine = { render: async () => "<html><body>page</body></html>" };
			initialize_render(engine, { is_dev: true });
			const ctx = { req: new Request("http://localhost:2339/"), request_url: "/", locale: "en-us", prefix: null, toasts: [], preferred_locale: null, translations: {} } as any;
			const html = await render_to_string("page", { ctx });
			expect(html).toContain("GITHUB_LABELS");
		});

		test("injects the issue reporter into main app pages", async () => {
			const engine = { render: async () => "<html><body>page</body></html>" };
			initialize_render(engine, { is_dev: true });
			const ctx = { req: new Request("http://localhost:2338/"), request_url: "/", locale: "en-us", prefix: null, toasts: [], preferred_locale: null, translations: {} } as any;
			const html = await render_to_string("page", { ctx });
			expect(html).toContain("GITHUB_LABELS");
		});
	});
	...
	describe("render - function", () => {
		...
		test("render redacts selected keys from development debug data", async () => {
			let captured_data: Record<string, any> | null = null;
			const engine = {
			...
			await render("page", {
				data: { env_content: "SECRET=value" },
				ctx,
				debug_redact_keys: ["env_content"],
			});

			const rendered_data = captured_data as unknown as Record<string, any>;
			expect(rendered_data.env_content).toBe("SECRET=value");
			const debug_data = JSON.parse(rendered_data.toJSON);
			expect(debug_data.env_content).toBe("[redacted]");
		});
