...
/**
 * `BunRequest` carries an already-parsed `CookieMap` (decoded values, `null`
 * for a missing name - the same contract as the manual parse below).
 *
 * The fallback is not dead weight: only requests routed through `Bun.serve`'s
 * router get `.cookies`. The generic upload endpoints run at the fetch-handler
 * level on a plain `Request` (see `lib/upload_endpoints.ts`), and the tests
 * construct plain `Request`s too, so both readers still have to cope with a
 * bare `Cookie` header.
 */
function cookie_map(req: BunRequest): Bun.CookieMap | null {
	const cookies = (req as { cookies?: Bun.CookieMap; }).cookies;
	return cookies && typeof cookies.get === "function" ? cookies : null;
}

// Minimal cookie reader for a specific cookie name.
export function get_cookie(req: BunRequest, name: string): string | null {
	const map = cookie_map(req);
	if (map) return map.get(name);
...
export function get_cookies_by_prefix(req: BunRequest, prefix: string): Array<{ key: string; value: string; }> {
	const map = cookie_map(req);
	if (map) { return [...map].filter(([key]) => key.startsWith(prefix)).map(([key, value]) => ({ key, value })); }
...
/**
 * Create a simple toast cookie for non-record-update messages (bulk-remove, filters, etc.).
 * Unlike `create_toast_cookie`, this doesn't resolve the session - it's synchronous.
 *
 * @param name  Cookie name (e.g. "toast-bulk-remove").
