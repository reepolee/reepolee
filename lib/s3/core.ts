...
// env_available() treats "N/A" (and absent/blank) as "not set", so a partial
// config like S3_HOSTNAME=real + S3_PROTOCOL=N/A falls back to the "http"
// default below instead of leaking the literal string "N/A" into the S3
// endpoint URL passed to Bun's native S3Client.
function read_s3_env(key: string): string {
	if (!env_available(key)) return "";
