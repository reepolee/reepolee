...
describe("get_filter_definitions", () => {
	const base_columns = {
		name: { width: "auto", class: "", filter: true, domain: "text" },
...
describe("resolve_filters", () => {
	...
	test("FK IN clause", () => {
		...
		expect(first.clause).toBe("author_id IN (?, ?)");
		expect(first.params).toEqual(["3", "5"]);
	});
	...
	test("FK NOT IN clause with ! prefix", () => {
		...
		expect(result.length).toBe(1);
		const first = result[0]!;
		expect(first.clause).toBe("author_id NOT IN (?, ?)");
		expect(first.params).toEqual(["3", "5"]);
	});
	...
	test("FK mixed IN and NOT IN", () => {
		...
		expect(first.clause).toBe("author_id NOT IN (?, ?)");
		expect(first.params).toEqual(["3", "7"]);
		expect(second.clause).toBe("author_id IN (?)");
		expect(second.params).toEqual(["5"]);
	});
	...
	test("boolean filter", () => {
		...
		expect(result.length).toBe(1);
		const first = result[0]!;
		expect(first.clause).toBe("is_customer = ?");
	...
	test("text filter uses LIKE with %term%", () => {
		...
		expect(result.length).toBe(1);
		const first = result[0]!;
		expect(first.clause).toBe("name LIKE ?");
		expect(first.params).toEqual(["%foo%"]);
	});
	...
	test("number filter exact match", () => {
		...
		expect(result.length).toBe(1);
		const first = result[0]!;
		expect(first.clause).toBe("vat_number = ?");
	...
	test("tags filter uses FIND_IN_SET", () => {
		...
		expect(result.length).toBe(1);
		const first = result[0]!;
		expect(first.clause).toBe("FIND_IN_SET(?, modules_tags) OR FIND_IN_SET(?, modules_tags)");
		expect(first.params).toEqual(["admin", "editor"]);
	});
...
describe("enrich_filter_definitions", () => {
	...
	test("splits options at 7 into visible and hidden, sets has_more", () => {
		const result = enrich_filter_definitions([fk_def], labels, {}, {}, all_options);
		const first = result[0]!;
		expect(first.visible_options).toHaveLength(7);
		expect(first.visible_options[0]!.option_text).toBe("Alice");
		expect(first.visible_options[6]!.option_text).toBe("Grace");
		expect(first.hidden_options).toHaveLength(2);
		expect(first.hidden_options[0]!.option_text).toBe("Hank");
		expect(first.hidden_options[1]!.option_text).toBe("Ivy");
	...
	test("has_more false when 7 or fewer options", () => {
		...
		const first = result[0]!;
		expect(first.visible_options).toHaveLength(5);
		expect(first.hidden_options).toHaveLength(0);
	...
	test("handles missing filter_options entry with empty arrays", () => {
		...
		const first = result[0]!;
		expect(first.visible_options).toEqual([]);
		expect(first.hidden_options).toEqual([]);
		expect(first.has_more).toBe(false);
	});
	...
	test("preserves extra FilterDef fields (fk_table, fk_column)", () => {
		const result = enrich_filter_definitions([fk_def], labels, {}, {}, all_options);
		const first = result[0]!;
		expect(first.fk_table).toBe("authors");
		expect(first.fk_column).toBe("id");
		expect(first.type).toBe("fk");
		expect(first.key).toBe("author_id");
	});
