...
// Route-map state is anchored to globalThis for the same reason as the
// translation repository (lib/i18n.ts) and the route table (lib/route_table.ts):
// bun --hot re-evaluates this module, while Bun.serve keeps the fetch closure
// captured at bootstrap. Per-module state meant reload_route_maps(), called
// from the pinned /__reload-translations endpoint, rebuilt maps that the live
// router never read. The state object itself is shared, so every generation
// reads and writes the same maps.
interface RouteMapState {
	current_maps: RouteMapSet | null;
	cached_routes: RouteTable | null;
	cached_locales: readonly string[] | null;
}

declare global {
	var __reepolee_route_map_state: RouteMapState | undefined;
}

if (!globalThis.__reepolee_route_map_state) {
	globalThis.__reepolee_route_map_state = { current_maps: null, cached_routes: null, cached_locales: null };
}

const state = globalThis.__reepolee_route_map_state;
...
export function build_route_maps(translations: Record<string, any>, routes: RouteTable, locales: readonly string[]): void {
	state.cached_routes = routes;
	state.cached_locales = locales;
	...
	state.current_maps = { by_locale, localized_patterns };
}

export function get_route_maps(): RouteMapSet {
	if (!state.current_maps) { throw new Error("Route maps not built yet. Call build_route_maps() first."); }
	return state.current_maps;
}

export function reload_route_maps(translations: Record<string, any>, routes?: RouteTable, locales?: readonly string[]): void {
	build_route_maps(translations, routes ?? state.cached_routes!, locales ?? state.cached_locales!);
}
...
export function detect_locale(localized_path: string): string | null {
	...
	for (const [locale, maps] of state.current_maps.by_locale) {
