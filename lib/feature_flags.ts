...
const REDIS_AVAILABLE = redis_available();
...
export async function add_to_allowlist(flag_name: string, ...user_ids: string[]): Promise<void> {
	if (!REDIS_AVAILABLE) return;
...
export async function remove_from_allowlist(flag_name: string, ...user_ids: string[]): Promise<void> {
	if (!REDIS_AVAILABLE) return;
	for (const id of user_ids) {
...
export async function get_flags(flag_names: string[], user_id: string): Promise<Record<string, boolean>> {
	...
	return Object.fromEntries(flag_names.map((name, i) => [name, results[i] ?? false]));
}
