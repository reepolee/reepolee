...
export type ServerStartOptions = {
	...
	// Explicit port override - used by the reeman app (apps/reeman/server.ts) so a
	// second process can bind its own REEMAN_PORT instead of the main app's PORT.
	port?: number;
};
...
export async function start_server(opts: ServerStartOptions): Promise<Bun.Server<WebSocketData>> {
	...
	const default_port = is_agent && Bun.env.AGENT_SERVER_PORT ? Number(Bun.env.AGENT_SERVER_PORT) : is_test ? Number(Bun.env.TEST_PORT) || 2600 : Number(Bun.env.PORT) || 2338;
