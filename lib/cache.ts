...
import { env_switch_on, redis_available } from "$config/env_vars";

// ---------------------------------------------------------------------------
// Guard
// ---------------------------------------------------------------------------

const CACHE_ENABLED = env_switch_on("CACHE_ENABLED");

if (CACHE_ENABLED && !redis_available()) {
	console.error("\u001b[31m✗ CACHE_ENABLED=true requires REDIS_ENABLED=true and a real REDIS_URL\u001b[0m");
	process.exit(1);
}

// Built on first use, never at import time: constructing a `RedisClient`
// parses REDIS_URL immediately, which throws on "N/A" or blank - the exact
// values that mean "cache disabled" for a Redis-free install. Lazy
// construction keeps those installs booting (mirrors
// lib/middleware/rate_limit_store.ts).
let default_redis_client: RedisClient | null = null;

function get_default_redis(): RedisClient {
	if (!default_redis_client) { default_redis_client = new RedisClient(Bun.env.REDIS_URL?.trim()); }
...
async function scan_all_keys(redis_client: RedisClient, pattern: string, count: number = 100): Promise<string[]> {
...
/**
 * Build a cache bound to a Redis client.
 *
 * `enabled` defaults to the module-level `CACHE_ENABLED`, so production
 * behaviour is unchanged. It is a parameter because reading the env only at
 * import time made the flag depend on which module happened to load this file
 * first - which is invisible in production but decided the value for the whole
 * test run, since `mock.module` is process-global in Bun.
 */
export function create_cache(redis_client: RedisClient | null, enabled: boolean = CACHE_ENABLED) {
	return {
		/**
		...
		is_enabled(): boolean { return enabled; },
	};
}
...
// Only constructed when caching is actually enabled: the guard above already
// guarantees Redis is available (REDIS_ENABLED=true and a real REDIS_URL)
// whenever CACHE_ENABLED is on, so a Redis-free install (CACHE_ENABLED off,
// REDIS_ENABLED=false and/or REDIS_URL="N/A" or unset) never constructs a
// RedisClient at all.
export const cache = create_cache(CACHE_ENABLED ? get_default_redis() : null, CACHE_ENABLED);
