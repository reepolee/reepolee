...
function resolve_values(
	...
	for (const field of fields) {
		for (const locale_code of active_locales as readonly string[]) {
...
export function build_localization_props(options: BuildLocalizationOptions): LocalizationProps {
	...
	// the order it happens to sit in active_locales.
	const ordered_locales = [default_locale, ...active_locales.filter((locale) => locale !== default_locale)];
...
export function parse_localized_form(params: URLSearchParams, fields: readonly LocalizedFormField[]): Record<string, Record<string, string>> {
	...
	const allowed = new Set((active_locales as readonly string[]).filter((locale) => locale !== default_locale));
...
export function parse_copy_request(params: URLSearchParams): CopyRequest | null {
	...
	const allowed = active_locales as readonly string[];
