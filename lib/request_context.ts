...
import { resolve_locale, route_namespace_from_dir, route_template_namespace_from_dir } from "$lib/route";
...
export class RequestContext {
	...
	// Translation namespace relative to the app roots (e.g. "reeman.users").
	route_dir: string | null = null;

	// Template path relative to the views root or mounted module root
	// (e.g. "users"). Mounted app prefixes are translation-only.
...
export async function create_ctx(req: BunRequest, meta_dir?: string): Promise<RequestContext> {
	...
	// Compute translation and template paths from caller's __dirname if provided.
	if (meta_dir) {
		ctx.route_dir = route_namespace_from_dir(meta_dir);
		ctx.template_dir = route_template_namespace_from_dir(meta_dir);
	}
