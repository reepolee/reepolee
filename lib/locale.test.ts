...
describe("locale helpers", () => {
	test("locale_language extracts the language part", () => {
		expect(locale_lib.locale_language("de-at")).toBe("de");
	});

	test("locale_region extracts the region part", () => {
		expect(locale_lib.locale_region("de-at")).toBe("at");
	});

	test("locale_url_segment lowercases", () => {
		expect(locale_lib.locale_url_segment("DE-AT")).toBe("de-at");
	});
