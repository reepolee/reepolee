	...
	for (const def of filter_definitions) {
		...
		try {
			const records = await timed_query(namespace, "load_tags_filter_options", async () => {
				...
				const result = await db`SELECT key_path, translation FROM translations WHERE namespace = ${namespace} AND key_path LIKE ${pattern} AND locale = ${locale} ORDER BY translation ASC`;
