...
import { flatten_translation_object, read_namespace_file } from "$lib/translation_files";
...
export interface EnrichedFilterDef extends FilterDef {
	...
	visible_options: { option_value: number | string; option_text: string; }[];
	hidden_options: { option_value: number | string; option_text: string; }[];
	has_more: boolean;
...
export function enrich_filter_definitions(
	...
	filter_options: Record<string, { option_value: number | string; option_text: string; }[]>,
): EnrichedFilterDef[] {
 ...
 * - Otherwise -> type "text"
 */
export function get_filter_definitions(columns: Record<string, { width?: string; class?: string; domain?: string; filter?: boolean; grid?: boolean; }>, fields: Record<string, FormFieldDef>): FilterDef[] {
...
/**
 * Load filter options for tags columns from translation files.
 *
 * Tags fields (e.g. `modules_tags`) contain comma-separated keys like "admin,default".
 * Their display labels come from translation files, not from the referenced DB table.
 *
 * Translations are stored under the route namespace, for example
 * `apps/reeman/users/en-us.json` with key `modules_tags.admin`.
 *
 * @param namespace - The route's translation namespace (e.g. "system.users")
 * @param locale - The current locale code (e.g. "en-us")
 */
export async function load_tags_filter_options(filter_definitions: FilterDef[], fields: Record<string, FormFieldDef>, namespace: string, locale: string): Promise<Record<string, { option_value: string; option_text: string; }[]>> {
	...
	for (const def of filter_definitions) {
		...
		try {
			const obj = await read_namespace_file(namespace, locale);
			const rows = flatten_translation_object(obj);
			const prefix = `${def.key}.`;
			const matching_rows = rows.filter((row) => row.key_path.startsWith(prefix));
			matching_rows.sort((left, right) => left.translation.localeCompare(right.translation));
			const records = matching_rows.map((row) => ({
				option_value: row.key_path.slice(prefix.length),
				option_text: row.translation,
			}));
