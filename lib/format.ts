 ...
 *
 * Kept for the routes that genuinely delete rows - see the reeman translations
 * bulk delete. Archivable CRUD routes use format_bulk_archive_message instead.
 */
...
/**
 * Format a bulk archive result message using pipe-separated plural translation keys.
 *
 * The archive counterpart of format_bulk_delete_message, reading
 * `msg.bulk_archived` instead of `msg.bulk_deleted`. Archivable CRUD routes no
 * longer delete rows, so telling the user "3 records deleted" would be a lie.
 */
export function format_bulk_archive_message(
	msg: { bulk_archived?: string; bulk_errors?: string; },
	archived_count: number,
	error_count: number,
	label: string = "record",
	locale: string = "en-us",
): string {
	const archived_key = msg.bulk_archived || `No ${label}s archived|{count} ${label} archived|{count} ${label}s archived|{count} ${label}s archived|{count} ${label}s archived`;
	const archived_part = plural(archived_key, archived_count, locale);

	if (error_count > 0) {
		const errors_key = msg.bulk_errors || `|{count} ${label} failed|{count} ${label}s failed|{count} ${label}s failed|{count} ${label}s failed`;
		const errors_part = plural(errors_key, error_count, locale);
		return `${archived_part}, ${errors_part}`;
	}
