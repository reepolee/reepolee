...
// The mock replaces $config/db in Bun's process-global registry, so it is
// what EVERY later test file in the run resolves $config/db to. It has to
// mirror the real module's whole export surface: returning a bare { db }
// erased DB_CONNECTION_STRING, and lib/middleware/rate_limit_store_sql.ts
// reads that at import time to pick its SQL dialect.
...
// `mock.module` replaces the module in Bun's process-global registry, so this
// stub is what EVERY later test file in the run resolves `$lib/cache` to - not
// just this one. Returning a bare `{ cache }` therefore erased `create_cache`
// for lib/cache.test.ts, whose 20 tests then failed with "create_cache is not a
// function" in a full run while passing in isolation. Spread the real module so
// only `cache.invalidate` is swapped and every other export survives.
const real_cache = await import("$lib/cache");
mock.module("$lib/cache", () => ({ ...real_cache, cache: { ...real_cache.cache, invalidate: async () => {} } }));
