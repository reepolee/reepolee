...
export function date_to_instant(date: Date): Temporal.Instant { return Temporal.Instant.from(date.toISOString()); } // Format a Date as a local-time "YYYY-MM-DD HH:MM:SS" string (sv-SE locale trick).
	// sv-SE is an Intl formatting idiom, not app locale identity - case is irrelevant to Intl.
 ...
 * -> 2026-05-20T02:02
 */
export function sql_to_datetime_local(value: string | Date | null | undefined, timezone = require_env("TIME_ZONE")): string {
	if (!value) return "";
