...
// `env_available()` maps the exact "N/A" marker (and an absent value) to "not
// configured", so TRUST_PROXY=N/A behaves like unset instead of an invalid mode.
function trusted_proxy(): string {
	if (!env_available("TRUST_PROXY")) return "";
	return Bun.env.TRUST_PROXY?.trim().toLowerCase() || "";
}
...
function socket_ip(req: BunRequest): string {
	const server = globalThis.__reepolee_server as Bun.Server<WebSocketData> | undefined;
	if (!server) return "";
...
export async function get_rate_limit_status(): Promise<{
	...
	return { scopes, total_keys: counters.length };
}
 ...
 *
 * The store is resolved by config: Redis when it is available (`REDIS_ENABLED=true`
 * and a real `REDIS_URL`), SQL otherwise. A Redis-free install is a supported
 * production configuration - there is no silent fallback to a pass-through
 * limiter either way.
 */
export function rate_limit_mw(store?: RateLimitStore): Middleware {
	...
	if (rate_limiting_on) {
		...
		if (!rl) {
			console.error("\u001b[31m✗ RATE_LIMITING=true requires a rate limit store (REDIS_URL or a SQL connection string)\u001b[0m");
			process.exit(1);
		}
