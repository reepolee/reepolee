...
type Sql_rate_limit_store = RateLimitStore & {
...
/**
 * Build a store against an explicit SQL client. Keeping the client injectable
 * also makes the dialect decision local to the connection being exercised;
 * Bun's test module mocks are process-global when tests run without
 * `--parallel`.
 */
export function create_sql_rate_limit_store(database: typeof db, connection_string: string): Sql_rate_limit_store {
	const is_mysql = connection_string.toLowerCase().startsWith("mysql://");
 ...
 */
	async function expire(key: string, seconds: number): Promise<void> {
		...
		await database`UPDATE rate_limit_counters SET expires_at = ${expires_at} WHERE counter_key = ${key}`;
	}
 ...
 */
	async function get(key: string): Promise<string | null> {
		const now = now_epoch_ms();
		const rows = await database`SELECT count FROM rate_limit_counters WHERE counter_key = ${key} AND expires_at > ${now}`;
		const row = rows[0];
 ...
 */
	async function reset_all(): Promise<number> {
		const result = await database`DELETE FROM rate_limit_counters`;
 ...
 */
	async function list_all(): Promise<Array<{ key: string; count: number; ttl: number; }>> {
		const now = now_epoch_ms();
		const rows = await database`SELECT counter_key, count, expires_at FROM rate_limit_counters WHERE expires_at > ${now}`;
 ...
 */
	async function cleanup_expired(): Promise<number> {
		const now = now_epoch_ms();
		const result = await database`DELETE FROM rate_limit_counters WHERE expires_at <= ${now}`;
		return affected_rows(result);
	}

	return { incr, expire, get, reset_all, list_all, cleanup_expired };
}

const store = create_sql_rate_limit_store(db, DB_CONNECTION_STRING);

export default store;

export const { cleanup_expired, expire, get, incr, list_all, reset_all } = store;
