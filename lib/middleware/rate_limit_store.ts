 ...
 * it is available (REDIS_ENABLED=true and a real REDIS_URL), SQL otherwise.
 * Call sites depend on the RateLimitStore contract only, so neither backend
...
export function is_redis_backed(): boolean { return redis_available(); }
