...
mock.module("$lib/route", () => ({ localized_url: (path: string, locale: string) => `/${locale}${path}` }));
...
describe("require_auth_mw middleware", () => {
	...
	test("uses locale from X-Locale header", async () => {
		const mw = require_auth_mw();
		const req = make_req({ url: "http://localhost/dashboard", headers: { "x-locale": "es-es" } });
		const next = make_next();

		const res = await mw(req, next);

		const location = res.headers.get("location");
		expect(location).toContain("/es-es/login");
	});

	test("uses locale from cookie if no X-Locale header", async () => {
		const mw = require_auth_mw();
		const req = make_req({ url: "http://localhost/dashboard", headers: { "cookie": "locale=fr-fr" } });
		const next = make_next();

		const res = await mw(req, next);

		const location = res.headers.get("location");
		expect(location).toContain("/fr-fr/login");
	});

	test("defaults to default_locale when no lang info", async () => {
		const mw = require_auth_mw();
		const req = make_req({ url: "http://localhost/dashboard" });
		const next = make_next();
		...
		const location = res.headers.get("location");
		expect(location).toContain("/en-us/login");
	});

	test("sets X-Locale-Preferred from cookie for authenticated users", async () => {
		const mw = require_auth_mw();
		const req = make_req({ headers: { "x-user-id": "admin", "cookie": "locale=es-es" } });
		...
		expect(captured_req.headers.get("x-locale-preferred")).toBe("es-es");
	});
});
...
describe("require_module_mw middleware", () => {
	...
	test("uses language from cookie for unauthenticated redirect", async () => {
		const mw = require_module_mw("admin");
		const req = make_req({ url: "http://localhost/admin", headers: { "cookie": "locale=es-es" } });
		const next = make_next();
	...
	test("sets X-Locale-Preferred for authorized users", async () => {
		const mw = require_module_mw("admin");
		const req = make_req({ headers: { "x-user-id": "admin", "cookie": "locale=fr-fr" } });
		let captured_req: any;
		const next = async (r: any) => {
			captured_req = r;
			return new Response("OK");
		...
		expect(captured_req.headers.get("x-locale-preferred")).toBe("fr-fr");
	});
