...
describe("move_styles_and_scripts_to_head with GROUP_JS disabled", () => {
	const html = `<html><head></head><body><script src="a.js"></script><script src="b.js" defer></script></body></html>`;

	test("relocates scripts to head without bundling them when GROUP_JS=false", async () => {
	...
	test("relocates scripts to head without bundling them when GROUP_JS is unset (default off)", async () => {
		const previous = process.env.GROUP_JS;
		delete process.env.GROUP_JS;
		try {
			const result = await move_styles_and_scripts_to_head(html);
			expect(result).toContain('<script src="a.js"></script>');
			expect(result).toContain('<script src="b.js" defer></script>');
			...
			if (previous === undefined) delete process.env.GROUP_JS;
			else process.env.GROUP_JS = previous;
		}
	});
});
