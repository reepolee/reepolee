...
// On by default: groups external <script src> tags into one immediate and
// one deferred bundle per page. Set GROUP_JS=false to leave each script tag
// as its own request (styles/links still relocate to head either way).
// Read lazily (not cached at module load) so tests can toggle it per-file.
function is_group_js_enabled(): boolean {
	return Bun.env.GROUP_JS !== "false";
}
