...
export function internal_admin_endpoints_enabled(): boolean {
	return env_switch_on("INTERNAL_ADMIN_ENDPOINTS");
}
...
export function require_admin_auth(req: Request, action_name: string): AuthOutcome {
	...
	// A misconfigured secret and a wrong secret both end in 401, but they need
	// very different fixes. Report the effective length when the value is too
	// short: quotes in .env are stripped before this check, so a value that
	// looks long enough in the file can still fail here.
	if (!reload_secret) {
		const effective_length = Bun.env.RELOAD_SECRET?.trim().length ?? 0;
		const reason = effective_length === 0
			? "RELOAD_SECRET is not set"
			: `RELOAD_SECRET is ${effective_length} chars, need >= ${MIN_RELOAD_SECRET_LENGTH} (surrounding quotes in .env are not counted)`;
		console.log(`[${action_name}] Misconfigured: ${reason}`);
		return { ok: false, response: new Response("Unauthorized", { status: 401 }) };
	}

	if (!secrets_match(header_secret, reload_secret)) {
