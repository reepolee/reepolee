...
| `route_module.ts`    | Barrel-exported modules and external development route mounts |
| `route_state.ts`     | Route registration state                                    |
| `request_context.ts` | `create_ctx()` -> `RequestContext` (locale, user, toasts)   |
...
`rate_limit_mw()` -> `set_locale()` -> `csrf_mw()`, where `csrf_mw()` is only appended when
...
| `set_locale.ts`       | Locale resolution per request                              |
| `csrf.ts`             | Double-submit CSRF token validation                        |
| `cors.ts`, `timing.ts`, `require_module_mw.ts` | CORS, timing, module gating       |

## i18n / translations

`i18n.ts` - `TranslationRepository` (exported as `translations`): file-first merge model,
...
| File                  | Role                                                       |
| --------------------- | ---------------------------------------------------------- |
| `resolve_db_type.ts`  | Auto-detect MySQL vs SQLite from the active connection string |
| `sql_runner.ts`       | `run_sql` dev SQL runner (`unsafe()` -> `{ meta, records }`) + guarded `run_sql_read_only` (single SELECT, result cap); `prepare_read_only_query` shared with the MCP inspection query |
