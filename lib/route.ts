...
import { resolve_route_module_namespace } from "$lib/route_module";
...
export function route_namespace_from_dir(dir: string): string {
	...
	if (idx === -1) {
		...
		throw new Error(`route_namespace_from_dir(): path is not under routes or a mounted route module: ${dir}`);
	}
