...
// ---------------------------------------------------------------------------
// The cache is constructed enabled explicitly, rather than by staging
// CACHE_ENABLED around the import.
//
// cache.ts reads that env var once, at import. Staging it here only worked
// while this file was the first to load the module - and it is not, in a full
// run, because a sibling test file loads it earlier. That made these tests pass
// alone and fail in the suite. Passing the flag removes the ordering
// dependency entirely.
// ---------------------------------------------------------------------------
...
function fresh_cache() { return create_cache(redis, true); }
...
run("cache.get_status", () => {
	...
	test("handles Redis error gracefully", async () => {
		await redis.close();
		try {
			const c = create_cache(new RedisClient(TEST_REDIS_URL!), true);
