...
/**
 * Read an image's stored width/height.
 *
 * `Bun.Image.metadata()` decodes only the header, so this is a plain read with
 * no subprocess in the common case (JPEG/PNG/WebP are statically linked into
 * the runtime, so they work on every platform).
 *
 * `autoOrient` is pinned **off** deliberately. Bun applies EXIF Orientation by
 * default, which reports 480x640 for a 640x480 JPEG tagged "rotate 90". Every
 * consumer of this function feeds the numbers to a vips CLI call - and vips
 * does not auto-rotate - so the *stored* dimensions are the ones that make the
 * resize scale factor and the crop region correct.
 *
 * Header reads Bun cannot do (TIFF on Linux, an unrecognised sniffer, or a
 * canvas over its pixel cap) fall through to `vipsheader`, which still backs
 * the rest of this module. Uploads accept any `image/*`, so that tail is real.
 */
export async function get_image_dims(file_path: string): Promise<{ width: number; height: number; }> {
	...
	try {
		({ width, height } = await Bun.file(safe_path).image({ autoOrient: false }).metadata());
	} catch (err) {
		const code = (err as { code?: string; }).code;
		// Missing/unreadable files surface the syscall code (ENOENT, EACCES) and
		// are not something vips can recover from either - rethrow those as-is.
		if (!code?.startsWith("ERR_IMAGE_")) { throw err; }

		log_warn("image_processor", "get_image_dims: Bun.Image could not read header, falling back to vipsheader", {
			file_path: safe_path,
			code,
		});
		({ width, height } = await get_image_dims_vipsheader(safe_path));
	}
...
// Fallback header read for formats Bun.Image cannot decode on this platform.
async function get_image_dims_vipsheader(safe_path: string): Promise<{ width: number; height: number; }> {
