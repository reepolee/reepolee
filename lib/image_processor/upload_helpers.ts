/**
 * Upload helpers - table-agnostic image editor helpers.
 *
 * Moved here from `apps/reeman/images/editor_helpers.ts` (M1 of
 * PLAN_image_file_manager_extraction.md): these functions have no `images`
 * table awareness, so regular app routes can reuse them for per-field
 * image upload/edit without touching reeman's admin CRUD.
 */
...
/**
 * Save the uploaded image from the form to a temporary location.
...
export async function save_upload_to_temp(form_data: FormData): Promise<Upload> {
	...
	if (file.size > max_upload_size_mb * 1024 * 1024) { throw new Error(`File too large: ${(file.size / 1024 / 1024).toFixed(1)} MB (max ${max_upload_size_mb} MB)`); }
