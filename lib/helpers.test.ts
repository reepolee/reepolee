...
describe("helpers", () => {
	...
	describe("get_locale_from_request", () => {
		test("reads X-Locale header", () => {
			const req = mock_req({ "x-locale": "sl-si" });
			expect(helpers.get_locale_from_request(req)).toBe("sl-si");
		});

		test("returns undefined when no header", () => {
			const req = mock_req({});
			expect(helpers.get_locale_from_request(req)).toBeUndefined();
		});

		test("canonicalizes casing", () => {
			const req = mock_req({ "x-locale": "en-us" });
			expect(helpers.get_locale_from_request(req)).toBe("en-us");
		});
	...
	describe("route_namespace_from_dir", () => {
		test("extracts route namespace from absolute path", () => {
			const result = helpers.route_namespace_from_dir(`/project/${MAIN_APP_POSIX}/auth/login`);
			expect(result).toBe("auth/login");
		});

		test("handles paths without subdirectories", () => {
			const result = helpers.route_namespace_from_dir(`/project/${MAIN_APP_POSIX}`);
			expect(result).toBe("");
		});

		test("extracts from Windows-style path", () => {
			const windows_root = MAIN_APP_POSIX.replaceAll("/", "\\");
			const result = helpers.route_namespace_from_dir(`C:\\project\\${windows_root}\\home`);
	...
	describe("merge_fields", () => {
		test("merges field definitions with overrides", () => {
			const generated: any = { name: { type: "text", label: "Name", attributes: { maxlength: 255 } } };
			const overrides: any = { name: { label: "Full Name", attributes: { required: true } } };
			const result: any = helpers.merge_fields(generated, overrides);
		...
		test("returns generated fields unchanged when no overrides", () => {
			const generated: any = { name: { type: "text", label: "Name" } };
			const result: any = helpers.merge_fields(generated, {});
	...
	describe("route_namespace_from_dir edge cases", () => {
		test("throws for a path outside the app roots and mounted modules", () => expect(() => helpers.route_namespace_from_dir("/some/other/path")).toThrow("path is not under"));

		test("handles path ending at the main app root", () => {
			const result = helpers.route_namespace_from_dir(`/project/${MAIN_APP_POSIX}`);
			expect(result).toBe("");
		});

		test("resolves the shared platform tree to a plain namespace", () => {
			expect(helpers.route_namespace_from_dir(`/project/${PLATFORM_DIR}/auth/login`)).toBe("auth/login");
		});
	});
