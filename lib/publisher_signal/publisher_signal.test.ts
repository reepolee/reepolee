...
describe("send_publisher_signal", () => {
	test("does nothing when the URL is not configured", async () => {
		...
		await send_publisher_signal("", fetcher, true);

		expect(calls).toBe(0);
	});

	test("does nothing when disabled, even with a URL configured", async () => {
		let calls = 0;
		const fetcher = async () => {
			calls++;
			return new Response(null, { status: 202 });
		...
		await send_publisher_signal("http://publisher.local/api/render-signal", fetcher, false);
