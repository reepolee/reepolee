...
mock.module("$config/supported_locales", () => ({
	...
	locale_names: { "en-us": "EN", "sl-si": "SL", "de-at": "DE", "nl-nl": "NL" },
	locale_aliases: {},
}));
...
describe("build_localization_props", () => {
	...
	test("includes inactive locales so content can be prepared before launch", () => {
		const props = build_localization_props({ fields: FIELDS, record: {}, copy_action: "/frameworks/1/copy-locale" });
		expect(props.active_locales).toContain("nl-nl");
	});
...
describe("parse_localized_form", () => {
	...
	test("collects values for inactive preparation locales", () => {
		const params = new URLSearchParams();
		params.set("_lv[name][nl-nl]", "Ripoli NL");
		expect(parse_localized_form(params, FIELDS)["nl-nl"]).toEqual({ name: "Ripoli NL" });
	});
