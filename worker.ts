...
// Processes translation jobs enqueued by `bun reeman sync-translations --translate`.
...
start_worker(
	...
	async (job) => {
		...
		const target_lang_name = locale_names[lang] ?? lang;
		...
		const read_current = (async (): Promise<any> => {
			const current: any = {};
			try {
				const rows = (await db`SELECT key_path, translation FROM translations WHERE namespace = ${namespace} AND locale = ${lang}`) as {
					key_path: string;
					translation: string;
				}[];
		...
		async function write_flat_to_db(db: any, ns: string, lg: string, obj: any) {
			...
			try {
				for (const [key_path, value] of flat) {
					try {
						await conn`DELETE FROM translations WHERE locale = ${lg} AND namespace = ${ns} AND key_path = ${key_path}`;
						await conn`INSERT INTO translations (locale, namespace, key_path, translation) VALUES (${lg}, ${ns}, ${key_path}, ${value})`;
					} catch {
						// Skip errors
					}
				}
