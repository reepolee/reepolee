...
CREATE TRIGGER web_push_subscriptions_updated_at_trigger
AFTER UPDATE ON web_push_subscriptions FOR EACH ROW
WHEN NEW.updated_at IS OLD.updated_at BEGIN
    UPDATE web_push_subscriptions SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;
