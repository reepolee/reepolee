...
CREATE TRIGGER developers_updated_at_trigger AFTER UPDATE ON developers FOR EACH ROW WHEN NEW.updated_at IS OLD.updated_at BEGIN
    UPDATE developers
    SET
...
INSERT INTO main.developers (id, name) VALUES
(1,'Aleš'),
(2,'Vishal'),
(3,'Evan'),
(4,'Jordan'),
...
CREATE TABLE frameworks (
    ...
    developer_id        INTEGER   NOT NULL REFERENCES developers(id) ON UPDATE CASCADE,
    reviewer_id         INTEGER   NOT NULL REFERENCES developers(id) ON UPDATE CASCADE,
    language_id         INTEGER   NOT NULL REFERENCES languages(id) ON UPDATE CASCADE,
    first_commit_on     TEXT      NOT NULL,
...
INSERT INTO main.frameworks (id, name, tagline, developer_id, reviewer_id, language_id, first_commit_on, is_javascript) VALUES
...
SELECT
    f.id,
    CAST(f.name AS TEXT) AS display,
    f.name,
    f.tagline,
    f.first_commit_on,
    f.is_javascript,
...
--       "name": "text",
--       "tagline": "longtext",
--       "developer_id": "foreign_key",
--       "reviewer_id": "foreign_key",
--       "language_id": "foreign_key",
