...
CREATE TABLE books (
    ...
    option_display      TEXT     GENERATED ALWAYS AS(title || CASE WHEN COALESCE(author, '')= '' THEN '' ELSE ' - ' || author END) VIRTUAL,
...
CREATE INDEX books_archived_at ON books(archived_at);

CREATE UNIQUE INDEX books_isbn_unique ON books(isbn);

CREATE TRIGGER books_update_timestamp AFTER UPDATE ON books FOR EACH ROW BEGIN
    UPDATE books
    SET
