...
CREATE TABLE developers (
    ...
    name                VARCHAR(15)  NOT NULL DEFAULT '' COMMENT 'ICU',
    display             VARCHAR(15)  GENERATED ALWAYS AS(name) VIRTUAL,
...
CREATE INDEX developers_name ON developers(name);
CREATE INDEX developers_archived_at ON developers(archived_at);

INSERT IGNORE INTO developers (id, name) VALUES
(1,'Aleš'),
(2,'Vishal'),
(3,'Evan'),
(4,'Jordan'),
...
CREATE INDEX languages_archived_at ON languages(archived_at);
...
CREATE TABLE frameworks (
    id                  INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'ICU',
    name                VARCHAR(15)  NOT NULL COMMENT 'ICU',
    tagline             VARCHAR(100) NOT NULL DEFAULT '' COMMENT 'ICU',
    developer_id        INT UNSIGNED NOT NULL COMMENT 'ICUF',
    reviewer_id         INT UNSIGNED NOT NULL COMMENT 'ICUF',
    language_id         INT UNSIGNED NOT NULL COMMENT 'ICUF',
    first_commit_at     DATE         NOT NULL COMMENT '',
    is_javascript       TINYINT(1)   NOT NULL COMMENT '',
    display             VARCHAR(15)  GENERATED ALWAYS AS(name) VIRTUAL,
    created_at          TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    archived_at         TIMESTAMP    NULL DEFAULT NULL,
    archived_by_user_id INT UNSIGNED NULL DEFAULT NULL,
    CONSTRAINT fk_frameworks_developer_id FOREIGN KEY(developer_id) REFERENCES developers(id) ON UPDATE CASCADE,
    CONSTRAINT fk_frameworks_reviewer_id FOREIGN KEY(reviewer_id) REFERENCES developers(id) ON UPDATE CASCADE,
    CONSTRAINT fk_frameworks_language_id FOREIGN KEY(language_id) REFERENCES languages(id) ON UPDATE CASCADE
) COMMENT '';

CREATE INDEX frameworks_developer_id ON frameworks(developer_id);
...
CREATE INDEX frameworks_archived_at ON frameworks(archived_at);

CREATE UNIQUE INDEX frameworks_name_unique ON frameworks(name);

INSERT IGNORE INTO frameworks (id, name, tagline, developer_id, reviewer_id, language_id, first_commit_at, is_javascript) VALUES
...
SELECT
    ...
    f.developer_id        AS developer_id,
    f.reviewer_id         AS reviewer_id,
    f.language_id         AS language_id,
    f.first_commit_at     AS first_commit_at,
    f.is_javascript       AS is_javascript,
    ...
    a2.display            AS reviewer_display
FROM frameworks f
    LEFT JOIN developers d
        ON d.id = f.developer_id
    LEFT JOIN developers a2
