...
CREATE TABLE recipes (
    ...
    name         VARCHAR(100) NOT NULL DEFAULT '' COMMENT 'ICU',
    servings     INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '',
    instructions TEXT         NOT NULL COMMENT '',
...
INSERT IGNORE INTO recipes (id, name, servings, instructions) VALUES
...
CREATE TABLE ingredients (
    id         INT UNSIGNED  NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'ICU',
    name       VARCHAR(100)  NOT NULL DEFAULT '' COMMENT 'ICU',
    quantity   DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '',
    unit       VARCHAR(20)   NOT NULL DEFAULT '' COMMENT '',
    recipe_id  INT UNSIGNED  NOT NULL COMMENT 'ICUF',
    created_at TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP     NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_ingredients_recipe_id FOREIGN KEY(recipe_id) REFERENCES recipes(id) ON UPDATE CASCADE
) COMMENT '';

CREATE INDEX ingredients_name ON ingredients(name);
CREATE INDEX ingredients_recipe_id ON ingredients(recipe_id);

INSERT IGNORE INTO ingredients (id, name, quantity, unit, recipe_id) VALUES
