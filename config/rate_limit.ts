...
export type RateLimitScope = "global" | "login" | "register" | "password" | "invite" | "validation" | "image_transform";
...
export const rate_limit_rules: Record<RateLimitScope, RateLimitRule> = {
	...
	// GET image transforms on S3-mounted paths (?width/height/longest/format).
	// These decode + re-encode and write derived cache objects back to the
	// bucket, so they are throttled per identity even though they are GETs
	// (the generic middleware only rate-limits state-changing methods).
	image_transform: { max: 60, window_s: 60 },
};
