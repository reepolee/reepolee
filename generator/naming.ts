...
/**
 * Names of all locale-clone tables present in `all_table_names`, e.g.
 * `frameworks_sl_si` when both it and its base table `frameworks` exist.
 * Used to hide generated clones from table pickers - they are derived
 * output, not tables a user selects directly.
 */
export function locale_clone_table_names(all_table_names: readonly string[], locale_codes: readonly string[], default_locale_code: string): Set<string> {
	const name_set = new Set(all_table_names);
	const clones = new Set<string>();
	for (const locale_code of locale_codes) {
		if (locale_code === default_locale_code) continue;
		const suffix = `_${locale_table_segment(locale_code)}`;
		for (const name of all_table_names) {
			if (!name.endsWith(suffix)) continue;
			if (name_set.has(name.slice(0, -suffix.length))) clones.add(name);
		}
	}
	return clones;
}
