 ...
 * (the user is present and `sync-locale-tables` is one command away); the
 * server fails loud, because a drifted locale table silently serves wrong
 * data for one locale.
 */
 ...
 * repaired - throwing here would remove the only tool that can fix it), the
 * server fails loud, because generated CRUD relies on the display columns.
 */
...
async function introspect_database(): Promise<DdlCacheData> {
	...
	try {
		...
		const cache = { generated_at: new Date().toISOString(), db_type, tables };
