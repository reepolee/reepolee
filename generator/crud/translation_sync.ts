...
export async function sync_nav_translations(table_name: string, clean_prefix: string, is_nested: boolean, route_name: string = ""): Promise<void> {
	...
	try {
		...
		await db_cli`DELETE FROM translations WHERE locale = ${default_locale} AND namespace = ${nav_key} AND key_path = 'nav'`;
		await db_cli`INSERT INTO translations (locale, namespace, key_path, translation) VALUES (${default_locale}, ${nav_key}, 'nav', ${label})`;
...
export async function sync_crud_translations(
	...
	try {
		...
		await db_cli.unsafe(`DELETE FROM translations WHERE locale = ? AND namespace = ? AND key_path IN (${placeholders})`, [default_locale, namespace, ...key_paths]);

		const value_slots = rows.map(() => "(?, ?, ?, ?)").join(", ");
		const value_params = rows.flatMap((r) => [default_locale, namespace, r.key_path, r.value]);

		await db_cli.unsafe(`INSERT INTO translations (locale, namespace, key_path, translation) VALUES ${value_slots}`, value_params);
...
export async function sync_validation_translations(
	...
	try {
		...
		const existing_rows = (await db_cli.unsafe(
			`SELECT key_path FROM translations WHERE locale = ? AND namespace = ? AND key_path IN (${placeholders})`,
			[default_locale, namespace, ...key_paths]
		)) as { key_path: string; }[];
		...
		const value_params = new_rows.flatMap((k) => [default_locale, namespace, `errors.${k.key}`, k.value]);
