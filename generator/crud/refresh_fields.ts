...
import type { ColumnDef, FieldDef, ForeignKeyMap, ParentInfo } from "./types";
...
export async function refresh_fields(config: RefreshFieldsConfig): Promise<boolean> {
	const { table_name, route_dir, relative_dir, fields, v_fields, columns, foreign_keys, route_prefix, is_nested, parent_info, translate_in_args, template_tags } = config;
