...
export async function update_routes_ts(config: UpdateRoutesConfig): Promise<RouteRegistrarResult> {
	...
	if (is_nested) {
		const import_stmt = `import { ${crud_name} } from "$main/${import_path}";`;
		log_step(`Looking for import: ${import_stmt}`);
		const existing_import_regex = new RegExp(`import \\{ [^}]+ \\} from "\\$main/${import_path}";`);
		if (existing_import_regex.test(routes_content) && !routes_content.includes(import_stmt)) {
			log_step(`Replacing existing import for "$main/${import_path}" with: ${import_stmt}`);
	...
	if (!is_nested) {
	...
	} else if (routes_content.includes("// GEN:ROUTES:CHILD:START") && routes_content.includes("// GEN:ROUTES:CHILD:END")) {
		...
		log_step(`Nested CRUD detected but no GEN:ROUTES:CHILD markers in routes.ts`);
		console.log(
			`\n⚠  Nested CRUD generated for "${table_name}". To register routes, add a GEN:ROUTES:CHILD section to routes.ts:\n  // GEN:ROUTES:CHILD:START\n  ...${crud_name},\n  // GEN:ROUTES:CHILD:END`
		);
	}
