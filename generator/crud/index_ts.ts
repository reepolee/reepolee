...
export async function generate_index_ts(config: GenerateIndexConfig): Promise<string> {
	...
	const [header_imports, route_export, header, validate, index_get, index_post, new_get, edit_get, edit_post, list_strategy, view_import, select_imports, index_bulk_delete, copy_locale_post] = await Promise.all([
	...
	const crud_routes_import = is_nested ? "" : "import { feature_paths, is_list_return_url, redirect_from_referer, run_bulk_delete } from \"$lib/crud_routes\";\n";
	...
	const localization_import = localized
		? `import { copy_localized_values, get_locale_rows, stale_copy_notices } from "$lib/localized_copy";\nimport { build_localization_props, localized_input_form_state, parse_copy_request, parse_localized_form, validate_localized_inputs } from "$lib/localized_form";\nimport { active_locales } from "$config/supported_locales";\nimport { invalidate_all_locales, save_locale_values } from "$lib/locale_write";\n`
		: "";
	...
	// Single-record reads take the request's locale so the edit page - and the
	// JSON view of it - answer from that locale's table rather than the base one.
	const read_locale_arg = localized ? ", ctx.locale" : "";
	...
	const load_localization = localized
		? `const locale_rows = await get_locale_rows(TABLE_NAME, Number(record.id), active_locales);\n\tconst notices = stale_copy_notices(record, locale_rows, LOCALIZED_FIELD_NAMES);\n\tconst localization = build_localization_props({ fields: LOCALIZED_FIELDS, record, locale_rows, notices, copy_action: ${copy_action_expr} });`
		: "";
	...
	const localization_error_data = localized
		? `localization: build_localization_props({ fields: LOCALIZED_FIELDS, record: { ...existing_record, ...data }, values: localized_values, errors: localized_errors, copy_action: \`\${base_path()}/\${__route_param__}/copy-locale\` }),`
		: "";
	...
	const catch_localization_data = localized
		? `\n\t\t\t\tlocalization: build_localization_props({ fields: LOCALIZED_FIELDS, record: existing_record ? { ...existing_record, ...data } : data, values: localized_values, errors: {}, copy_action: \`\${base_path()}/\${__route_param__}/copy-locale\` }),`
		: "";
	...
	content = apply_template(
		content,
		{
			...
			"autocomplete.options_route": autocomplete_options_route + copy_locale_route,
			...
			// The edit handler tells update_record which locale it is editing, so
			// the fan-out writes localized columns to that locale's table only.
			...
			"edit.get_lookup": has_custom_route_param ? `const ${route_param_value} = req.params.${route_param_value} || "";\n\tconst record = await get_record_by_route_param(${route_param_value});` : is_auto_increment_pk ? `const id = Number(req.params.id || 0);\n\tconst record = await get_record_by_id(id${read_locale_arg});` : `const id = req.params.id ? String(req.params.id) : "";\n\tconst record = await get_record_by_id(id${read_locale_arg});`,
