...
export async function load_table_schema(table_name: string, options: {
	...
	const { clean_prefix, route_prefix, parent_cli_table, route_name: raw_route_name, pagination_strategy: cli_pagination, template_tags: cli_template_tags } = options;
	...
	try {
		...
		column_names = get_cached_table(cache, table_name)?.columns.map((c) => c.name) ?? [];
	} catch (err) {
...
function backfill_grid_filler(schema_content: string, grid_filler: string): string | null {
	...
	const const_anchor = "const enable_delete = ";
	...
	const export_anchor = "export { columns, route_param, enable_delete,";
	...
	return with_const.replace(export_anchor, "export { columns, route_param, enable_delete, grid_filler,");
}
...
function localized_input_type(field: FieldDef): string {
	...
	if (field.type === "textarea" || field.type === "json") return "textarea";
