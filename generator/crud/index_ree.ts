...
import { find_v_field, has_archive_column } from "./helpers";
...
//
// Restore lives on the index because an archived row is otherwise reachable
// only by typing its edit URL. The grid row is a single <a> spanning the
// subgrid, so a per-row <form> cannot be nested inside it: one shared hidden
// form is retargeted at the clicked record and posts `_action=restore` to that
// record's edit route, the same endpoint the edit page's restore button uses.
// ---------------------------------------------------------------------------

/** Expression for the record's URL segment, matching the row link. */
function record_param_expr(route_param_value: string): string {
	return route_param_value === "id" ? "record.id" : `record.${route_param_value} || record.id`;
}

/** Per-row restore button, rendered only for a row that is actually archived. */
export function archive_row_restore(has_archive: boolean, route_param_value: string): string {
...
/**
 * Shared restore form, its confirm dialog and the retargeting script.
 *
 * The dialog is opened by hand rather than through dialog-confirm.js's
 * `command="show-modal"` handler: that helper is only loaded when bulk actions
 * are enabled, and the click has to set the form action before the dialog
 * appears anyway.
 */
function archive_restore_ui(has_archive: boolean, route_prefix: string, route_name: string): string {
	if (!has_archive) return "";
	const list_path = `{~ localized_path('${route_prefix}/${route_name}') }`;
	return `<form id="restore-form" method="POST" action="" style="display: none;">
		<input type="hidden" name="_csrf_token" value="{= csrf_token }" />
		<input type="hidden" name="_action" value="restore" />
		<input type="hidden" name="_return_url" id="restore_return_url" value="" />
	</form>
	...
	<script>
		function open_restore_dialog(record_param) {
			const restore_form = $("#restore-form");
			restore_form.action = "${list_path}/" + record_param + "/edit";
			$("#restore_return_url").value = window.location.pathname + window.location.search;
			$("#action_restore_dialog").showModal();
		}
...
export async function generate_index_ree(options: IndexReeOptions): Promise<{ index_html: string; rows_html: string | null; }> {
	const { table_name, singular, fields, v_fields = null, column_names = [], columns_override = null, route_prefix = "", route_param_value = "id", pagination_strategy = "cursor", render_strategy = "load", route_name = "" } = options;
	...
	if (columns_override) {
		...
		filtered = display_fields.filter((f) => !f.attributes?.omit && f.attributes?.omit_index !== true && !(IGNORE_INDEX_FIELDS as readonly string[]).includes(f.name));
	}
	...
	const deferred_stream_placeholders = [
		"stream.pagination_start",
		"stream.pagination_count",
	...
	// Archive UI is decided by the schema, exactly like the SQL layer: a table
	// without `archived_at` gets no markup for it at all.
	...
	html = apply_template(html, {
		...
		"table.cells": cells,
		"archive.counts_block": archive_counts_block(has_archive),
		"archive.row_restore": archive_row_restore(has_archive, route_param_value),
		"archive.restore_ui": archive_restore_ui(has_archive, route_prefix, effective_route_name),
		"archive.action_value": has_archive ? "archive" : "delete",
		"archive.bulk_failed": has_archive ? "Bulk archive failed" : "Bulk delete failed",
		route_prefix: route_prefix,
		route_param: route_param_value,
	}, { deferred: deferred_stream_placeholders });
	...
	if (render_strategy === "stream") {
		...
		html = apply_template(html, {
			...
			"stream.records_end": "",
		}, { deferred: ["stream.pagination_start", "stream.pagination_count", "stream.pagination_end"] });
