...
export async function search_records(search: string = "", offset: number = 0, limit: number = 20, order_by: string = "id::asc", scope_clause: string = "", filter_clauses: { clause: string; params: any[] }[] = []__sql.locale_param____sql.archive_param__): Promise<{ records: Record[], total: number }> {
	try {
		...
		if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(sort_field)) {
			return search_records(search, offset, limit, "id::asc", scope_clause, filter_clauses__sql.locale_arg____sql.archive_arg__);
		}

		return await timed_query("__table.exact__", "search_records", async () => {
			return cache.search(
				"__sql.route__",
				{ search, offset, limit, order_by, scope_clause, filter_clauses__sql.cache_key_locale____sql.archive_cache_key__ },
