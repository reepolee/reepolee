			...
			return cache.search(
				...
				async () => {
					...
					if (search) {
						...
						const data_query = `SELECT * FROM __table.exact__ WHERE ${search_where_clause} ORDER BY ${sort_field} ${valid_direction.toUpperCase()}, id ${valid_direction.toUpperCase()} LIMIT ? OFFSET ?`;
						records = await db.unsafe(data_query, [...search_params, limit, offset]) as Record[];

						const count_query = `SELECT COUNT(*) as count FROM __table.exact__ WHERE ${search_where_clause}`;
