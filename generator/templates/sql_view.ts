...
export async function get_all_records_view(search: string = "", after: string | null = null, before: string | null = null, is_last: boolean = false, limit: number = 20, order_by: string = "id::asc", scope_clause: string = "", filter_clauses: { clause: string; params: any[] }[] = []__sql.view_locale_param____sql.archive_param__): Promise<{ records: ViewRecord[], total: number }> {
	try {
		...
		if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(sort_field)) {
			return get_all_records_view(search, after, before, is_last, limit, "id::asc", scope_clause, filter_clauses__sql.locale_arg____sql.archive_arg__);
		}

		return await timed_query("__view.name__", "get_all_records_view", async () => {
			return cache.search(
				"__sql.route__",
				{ search, after, before, is_last, limit, order_by, scope_clause, filter_clauses__sql.view_locale_cache_key____sql.archive_cache_key__ },
				...
				async () => {
					...
					const params: any[] = [__sql.view_locale_filter_param__];
					...
					const data_query = `SELECT * FROM ${view_source} ${where} ORDER BY ${sort_field} ${query_direction.toUpperCase()}, id ${query_direction.toUpperCase()} LIMIT ?`;
					...
					if (!search) {
						const count_where_clauses: string[] = [__sql.view_locale_where__];
						__sql.archive_count_push__
						const count_params: any[] = [__sql.view_locale_filter_param__];
						if (scope_clause) {
