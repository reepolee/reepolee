...
export async function get_record_by_id(id: __sql.id_type____sql.locale_param__): Promise<Record | undefined> {
	try {
...
export async function search_records(search: string = "", after: string | null = null, before: string | null = null, is_last: boolean = false, limit: number = 20, order_by: string = "id::asc", scope_clause: string = "", filter_clauses: { clause: string; params: any[] }[] = []__sql.locale_param__): Promise<{ records: Record[], total: number }> {
	try {
		...
		if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(sort_field)) {
			return search_records(search, after, before, is_last, limit, "id::asc", scope_clause, filter_clauses__sql.locale_arg__);
		}

		return await timed_query("__table.exact__", "search_records", async () => {
			return cache.search(
				"__sql.route__",
				{ search, after, before, is_last, limit, order_by, scope_clause, filter_clauses__sql.cache_key_locale__ },
