...
export async function get_all_records_view(search: string = "", offset: number = 0, limit: number = 20, order_by: string = "id::asc", scope_clause: string = "", filter_clauses: { clause: string; params: any[] }[] = []__sql.view_locale_param__): Promise<{ records: ViewRecord[], total: number }> {
	try {
		...
		if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(sort_field)) {
			return get_all_records_view(search, offset, limit, "id::asc", scope_clause, filter_clauses__sql.locale_arg__);
		}

		return await timed_query("__view.name__", "get_all_records_view", async () => {
			return cache.search(
				"__sql.route__",
				{ search, offset, limit, order_by, scope_clause, filter_clauses__sql.view_locale_cache_key__ },
