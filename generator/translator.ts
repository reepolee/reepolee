	...
	for (let i = 0; i < chunks.length; i++) {
		...
		for (let attempt = 0; attempt <= max_retries; attempt++) {
			try {
				...
				const translated = await translate_attempt(chunk, targetLang, source_lang, model, timeout);
...
CRITICAL RULES:
- Translate the into pure language or locale as asked.
- Do not mix other similar or close languages' vocabulary or grammar, be strict and precise.
...
function log_translation_output(target_lang: string, output: Record<string, any>): void {
	const output_text = JSON.stringify(output, null, 2);
	console.log(`📥 Translation output (${target_lang}):\n${output_text}`);
}
