 ...
 *
 * Removes a language from config/supported_locales.ts, deletes DB translations,
...
export async function remove_locale_from_system(locale_code: string, options: RemoveLocaleOptions = {}): Promise<boolean> {
	...
	try {
		const [result] = await Promise.all([
			db_cli`DELETE FROM translations WHERE locale = ${locale_code}`,
			db_cli`DELETE FROM translations WHERE key_path = ${`ui.locale_names.${locale_code}`}`,
			db_cli`DELETE FROM translations WHERE key_path = ${`ui.language_names_to.${locale_code}`}`,
		]);
		const count = (result as any)?.affectedRows ?? (result as any)?.count ?? (result as any)?.changes ?? 0;
		console.log(`   ✓ Deleted ${count} translation(s) from DB for "${locale_code}"`);
