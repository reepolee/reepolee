...
export function read_fields(fields: FieldDef[], include_maintenance = true): FieldDef[] {
	return fields.filter((f) => {
		if (f.attributes?.omit) return false;
		if (!include_maintenance && (MAINTENANCE_FIELDS as readonly string[]).includes(f.name.toLowerCase())) return false;
		return true;
	});
}
...
export function generate_zod_fields_from_array(fields: FieldDef[], type: "index" | "form" | "validate", foreign_keys?: Map<string, any>): string {
	...
	for (const field of filtered_fields) {
		const has_fk = field.type === "foreign_key" || foreign_keys?.has(field.name) || field.attributes?.foreign_key;
		const is_maintenance = (MAINTENANCE_FIELDS as readonly string[]).includes(field.name.toLowerCase());
		...
		if (type === "index") {
			...
			} else if (field.type === "number" || field.type === "yes_no") {
			...
			if (has_fk) {
				...
				schema = `z.coerce.number().min(0, "${field.name}_required")`;
 ...
 * scanner in reeman/insert_translations.ts cannot find them. This function walks
...
export function collect_validation_error_keys(fields: FieldDef[], foreign_keys?: Map<string, any>): Array<{ key: string; value: string; }> {
	...
	for (const field of filtered_fields) {
		...
		if (has_fk) {
			...
			keys.set(`${field.name}_required`, `${label} is required.`);
