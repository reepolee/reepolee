...
async function update_root_nav_translations(db: SQL, table_names: string[], prefix: string = "", route_name: string = ""): Promise<void> {
	for (const name of table_names) {
		...
		try {
			await db`DELETE FROM translations WHERE locale = ${default_locale} AND namespace = ${nav_key} AND key_path = 'nav'`;
			await db`INSERT INTO translations (locale, namespace, key_path, translation) VALUES (${default_locale}, ${nav_key}, 'nav', ${label})`;
		} catch (err) {
...
async function update_route_label_translations(
	...
	try {
		await db`DELETE FROM translations WHERE locale = ${default_locale} AND namespace = ${namespace} AND key_path = ${key_path}`;
		await db`INSERT INTO translations (locale, namespace, key_path, translation) VALUES (${default_locale}, ${namespace}, ${key_path}, ${english_value})`;
	} catch (err) {
...
async function inject_scopes_translations(db: SQL, route_dir: string, table_name: string): Promise<void> {
	try {
		...
		for (const row of scope_rows) {
			...
			try {
				await db`DELETE FROM translations WHERE locale = ${default_locale} AND namespace = ${namespace} AND key_path = ${key_path}`;
				await db`INSERT INTO translations (locale, namespace, key_path, translation) VALUES (${default_locale}, ${namespace}, ${key_path}, ${row.display_name})`;
				injected_count++;
			} catch (err) {
