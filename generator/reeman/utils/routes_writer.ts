...
export function add_module_route_entry(routes_content: string, module_code: string): RouteInsertResult {
	const helper_import = `import { try_load_routes } from "$lib/route_module";`;
	if (!routes_content.includes(helper_import)) {
		throw new Error(`routes/routes.ts must import try_load_routes before modules can be registered.`);
	}
	...
	if (!routes_content.includes(system_marker)) {
		throw new Error(`Could not find the SYSTEM marker in routes/routes.ts.`);
	}
