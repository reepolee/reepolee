 ...
 * lookups and find file-backed keys that are no longer referenced.
 *
 * Exports:
 * - find_orphaned_keys() - core logic: scan .ree files, compare against locale files
 ...
 * 6. Optionally deletes confirmed orphans from locale files
 */
...
export interface OrphanResult {
	orphans: OrphanKey[];
	stats: { files: number; refs: number; file_pairs: number; };
}
...
function extract_all_refs(content: string): Set<string> {
	...
	// Locale-file key path - same shape namespace_from_path() + the ::-join expect.
 ...
 * and {_ ... }/{- ... } translation lookups against locale files.
 *
 * @param ree_dirs - directories to scan for .ree files
 * @param cwd - current working directory (used for namespace resolution)
 */
export async function find_orphaned_keys(ree_dirs: string[], cwd: string): Promise<OrphanResult> {
	...
	if (all_files.length === 0) { return { orphans: [], stats: { files: 0, refs: 0, file_pairs: 0 } }; }
	...
	// 3. Read existing (namespace, key_path) pairs from files
	const db_pairs: OrphanKey[] = [];
	const rows = await read_all_translation_rows(cwd);
	const unique_pairs = new Map<string, OrphanKey>();
	for (const row of rows) {
		const pair_key = `${row.namespace}::${row.key_path}`;
		if (!unique_pairs.has(pair_key)) unique_pairs.set(pair_key, { namespace: row.namespace, key_path: row.key_path });
	}
	db_pairs.push(...unique_pairs.values());

	// 4. Find file keys not referenced in any template
	...
	return {
		orphans,
		stats: { files: all_files.length, refs: in_use.size, file_pairs: db_pairs.length },
	};
}
...
export async function prune_unused_translations(): Promise<void> {
	...
	console.log(`  ${dim("Scans .ree templates and compares references against locale files.")}`);
	console.log();
	...
	console.log(`  ${dim(`${result.stats.file_pairs} unique (namespace, key_path) pairs in locale files.`)}`);
	console.log();
	...
	const apply_changes = await confirm(`Delete ${result.orphans.length} unused key(s) from locale files?`, "n");
	if (apply_changes) {
		const rows = await read_all_translation_rows(cwd);
		let deleted = 0;
		...
		console.log(`  ${color("✓", GREEN)} Deleted ${deleted} locale value(s).`);
	}
