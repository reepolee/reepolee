 ...
 * Re-reads routes.ts fresh so repeated calls (e.g. from a multi-select batch) never
 * operate on stale line indices from a previous deletion in the same run.
 */
async function remove_single_route(selected: ParsedEntry, del_translations_opt: boolean | undefined, force: boolean): Promise<void> {
	...
	const del_translations = del_translations_opt !== undefined
		? del_translations_opt
		: force
			? false
			: await confirm(`Delete all translation entries for namespace "${namespace || "(global)"}"${child_namespace_note} from DB?`, "n");
 ...
 */
export async function remove_route(url?: string, force: boolean = false, del_translations_opt?: boolean): Promise<void> {
	const routes_path = join(process.cwd(), "routes", "routes.ts");
	const raw = await Bun.file(routes_path).text();
	const sys_modules = ["system"];
	...
	if (url) {
		...
		await remove_single_route(selected, del_translations_opt, force);
		return;
	}
	...
	for (const entry of selected_entries) {
		...
		await remove_single_route(entry, del_translations_opt, force);
	}
}
