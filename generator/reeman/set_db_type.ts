...
export async function set_db_type(type?: "mysql" | "sqlite"): Promise<"mysql" | "sqlite" | null> {
	...
	if (type) {
		...
		const db_items = [
			{ value: "sqlite", label: "SQLite - DEV_CONNECTION_STRING=\"sqlite:app.db\"" },
			{
				value: "mysql",
				label: "MySQL / MariaDB - DEV_CONNECTION_STRING=\"mysql://user:pass@localhost/db\"",
			},
		];
	...
	// Update .env - just uncomment the correct DEV_CONNECTION_STRING and comment the other
	...
	if (existsSync(env_path)) {
		...
		console.log(`  ${color("No .env or .env.example found. Skipping DEV_CONNECTION_STRING update.", YELLOW)}`);
		return null;
	}

	const lines = env_content.split("\n");
	const target_keys = ["DEV_CONNECTION_STRING", "TEST_CONNECTION_STRING"];

	for (let i = 0; i < lines.length; i++) {
		const original = lines[i]!;
		const trimmed = original.trim();

		const conn_match = trimmed.match(/^(#\s*)?(?:export\s+)?(DEV_CONNECTION_STRING|TEST_CONNECTION_STRING)=(?:"([^"]*)"|'([^']*)'|(\S*))\s*$/);
	...
	console.log(`  ${color("✓", GREEN)} Updated .env \u2192 ${is_sqlite ? "sqlite" : "mysql"} active (DEV_CONNECTION_STRING + TEST_CONNECTION_STRING)`);
