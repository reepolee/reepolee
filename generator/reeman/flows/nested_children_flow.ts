...
export async function run_nested_children_flow(): Promise<boolean> {
	...
	const nested_template_tags = (await select_from_list("Select form field rendering mode", template_tags_items)) as "flat" | "tags" | "";

	if (nested_template_tags) {
		console.log(`  ${color("✓", GREEN)} Template tags: ${color(BOLD + nested_template_tags, CYAN)}`);
	} else {
		console.log(`  ${dim("  (default: flat)")}`);
	}
	...
	const template_tags = nested_template_tags || undefined;
	const parent_ok = await run_full_pipeline(parent_table, { prefix: nested_prefix, pagination_method, render_strategy, template_tags });
	...
	console.log(`  ${color("Template tags:", BOLD)} ${nested_template_tags || "flat"}`);
	console.log(`${color("-".repeat(50), CYAN)}\n`);
	...
	const tip_lines = [
		...build_full_pipeline_cli_tip({ table: parent_table, prefix: nested_prefix, pagination_method, render_strategy, template_tags }),
		...selected_children.flatMap((table) => build_full_pipeline_cli_tip({
			table,
	...
	show_cli_tips(tip_lines, `Nested CRUD generated: ${parent_table} -> ${selected_children.join(", ")}`);
