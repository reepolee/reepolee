...
export async function run_bulk_crud_flow(): Promise<boolean> {
	...
	const bulk_template_tags = (await select_from_list("Select form field rendering mode", template_tags_items)) as "flat" | "tags" | "";

	if (bulk_template_tags) {
		console.log(`  ${color("✓", GREEN)} Template tags: ${color(BOLD + bulk_template_tags, CYAN)}`);
	} else {
	...
	console.log(`  ${color("Template tags:", BOLD)} ${bulk_template_tags || "flat"}`);
	...
	const template_tags = bulk_template_tags || undefined;
	...
	const tip_lines = selected_tables.flatMap((table) => build_full_pipeline_cli_tip({
		table,
		prefix: bulk_prefix,
		pagination_method,
		render_strategy,
	...
	show_cli_tips(tip_lines, `Bulk CRUD generated: ${selected_tables.join(", ")}`);
