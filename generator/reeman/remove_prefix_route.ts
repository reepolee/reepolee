...
export async function remove_prefix_folder(name?: string, force: boolean = false, del_translations_opt?: boolean, as_examples: boolean = false): Promise<void> {
	...
	if (routes_file_exists) {
		...
		// (`import { route_definitions as user_clubs } from "$main/user/clubs";`)
		// and plain nested-crud imports (`import { user_members_crud } from "$main/user/members";`).
		...
		const filtered_lines = lines.filter((line) => {
			...
			if (trimmed.startsWith("import ") && (trimmed.includes(`/routes/${selected.name}/`) || trimmed.includes(`$main/${selected.name}/`))) {
