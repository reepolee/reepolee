...
export async function run_sql_file(): Promise<void> {
	...
	const conn_str_display = Bun.env.DEV_CONNECTION_STRING?.trim();
	if (conn_str_display) {
	...
	// Determine DB type from DEV_CONNECTION_STRING and prefer sql/{type}/ folder
...
export async function execute_sql_file(relative_path: string, force: boolean = false): Promise<boolean> {
	...
	const conn_str = Bun.env.DEV_CONNECTION_STRING?.trim();
	if (!conn_str) {
		console.log(`  ${color("DEV_CONNECTION_STRING not set in .env.", RED)}`);
		return false;
	}
