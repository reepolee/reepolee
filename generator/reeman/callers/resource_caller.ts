...
export async function run_bulk_generator(
	...
	for (let i = 0; i < tables.length; i++) {
		...
		const ok = await run_full_pipeline(table, { prefix, translate, pagination_method, render_strategy, template_tags });
		if (ok) {
			success_count++;
		} else {
			fail_count++;
...
export async function run_bulk_nested_generator(
	...
	for (let i = 0; i < tables.length; i++) {
		...
		const ok = await run_full_pipeline(table, { prefix, parent_table, pagination_method, render_strategy, translate, template_tags });
		if (ok) {
			success_count++;
		} else {
			fail_count++;
