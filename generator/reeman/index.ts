...
import { install_locale } from "./install_locale";
	...
	while (true) {
		...
		const menu_groups: MenuGroup[] = [
			{
				...
				options: [
					{
						cmd: "create_bread",
						label: "Fake-table resource (BREAD)",
						description: "Generate full CRUD for a config, filesystem, or API data source",
					},
					{
						cmd: "crud",
						label: "Single table",
				...
				options: [
					{
						...
						description: "Switch between MySQL and SQLite, update .env DEV_CONNECTION_STRING",
					},
					{
						cmd: "set_repo",
						label: "Set repository",
						description: "Link this project to a GitHub repo for dev-mode issue reports (package.json ree.issue_repo)",
					},
					{
						cmd: "run_sql_file",
						label: "Run SQL file",
			...
			},
			{
				...
				options: [
					...
					{
						cmd: "install_locale",
						label: "Install archived locale",
						description: "Install a locale from locales-archive/ (curated translations, no AI call)",
					},
					{
						cmd: "add_locale",
						label: "Add locale",
		...
		try {
			...
			const cmd_labels: Record<string, string> = {
				...
				schema: "Schema only",
				crud: "CRUD only",
				create_bread: "Fake-table resource (BREAD)",
				...
				set_repo: "Set repository",
				install_locale: "Install archived locale",
			...
			if (command === "install_locale") {
				await install_locale();
				await press_enter();
				continue;
			}
			...
			if (command === "create_bread") {
				await run_create_bread_flow();
				await press_enter();
				continue;
			}
