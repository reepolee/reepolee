 ...
 * translation lookups and add keys that are missing from locale files.
 *
 * Exports:
 * - find_missing_keys() - core logic: scan .ree files, compare against locale files
 * - write_missing_translations() - add missing keys to locale files
 * - insert_translations() - reeman wrapper with user prompts
 *
 * How it works:
 * 1. Scans all .ree files in routes/, public/, and components/
 ...
 * 3. Maps each file to its namespace from the file path
 * 4. Reads existing (namespace, key_path) pairs from locale files
 * 5. Keys referenced in templates but absent from both the namespace and
 * root fallback are flagged as missing
 * 6. Writes confirmed missing keys to all configured locale files
 */
...
import { read_all_translation_rows, upsert_file_translation } from "$lib/translation_files";

import { color, confirm, dim, GREEN, header, press_enter, YELLOW } from "./ui";
import { capitalize_label } from "../schema/field_generator";
import { MAIN_APP, MAIN_APP_SEGMENTS } from "$config/paths";
...
export interface MissingResult {
	missing: MissingKey[];
	stats: { files: number; translation_refs: number; file_pairs: number; };
}
...
function extract_all_refs(content: string): Set<string> {
	...
	// locale-file key path - e.g. {_ labels.email } -> "labels.email".
...
function namespace_from_path(file_path: string, cwd: string): string {
	...
	if (MAIN_APP_SEGMENTS.every((segment, index) => parts[index] === segment)) {
		const ns_parts = parts.slice(MAIN_APP_SEGMENTS.length, -1);
		return ns_parts.length > 0 ? ns_parts.join(".") : "root";
	}

	if (parts[0] === "components") return "root";
...
export async function find_missing_keys(ree_dirs: string[], cwd: string): Promise<MissingResult> {
	...
	if (all_files.length === 0) {
		return { missing: [], stats: { files: 0, translation_refs: 0, file_pairs: 0 } };
	}
	...
	// 3. Read existing (namespace, key_path) pairs
	...
	for (const [namespace, key_paths] of template_refs) {
		for (const key_path of key_paths) {
			...
			// Skip if exact match exists in a locale file
	...
	return {
		...
		stats: {
			...
			file_pairs: rows.length,
		},
	};
}

function cleaned_english_value(key_path: string): string {
	const path_segments = key_path.split(".");
	const key_name = path_segments.at(-1) ?? key_path;
	return capitalize_label(key_name);
}

export async function write_missing_translations(missing: MissingKey[], project_dir: string = process.cwd()): Promise<number> {
	let count = 0;
	for (const { namespace, key_path } of missing) {
		const english_value = cleaned_english_value(key_path);
		for (const locale of locales) {
			const translation = locale === default_locale ? english_value : `::missing:: ${english_value}`;
			await upsert_file_translation(locale, namespace, key_path, translation, project_dir);
			count++;
		}
	}
	return count;
...
export async function insert_translations(): Promise<void> {
	header("Sync missing translation keys");

	console.log(`  ${dim("Scans .ree templates for translation references missing from locale files.")}`);
	console.log();
	...
	console.log(`  ${dim(`${result.stats.file_pairs} locale values found in files.`)}`);
	console.log();

	if (result.missing.length === 0) {
		console.log(`  ${color("✓ All translation keys referenced in templates exist in locale files or root fallback. Nothing to sync.", GREEN)}`);
		return;
	}
	...
	const apply_changes = await confirm(`Add ${result.missing.length} missing key(s) to ${locales.length} locale files?`, "n");
	if (apply_changes) {
		const total_rows = await write_missing_translations(result.missing, cwd);
		console.log(`  ${color("✓", GREEN)} Added ${total_rows} locale value(s).`);
	}
