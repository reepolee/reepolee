 ...
 * for all active locales. The default English locale receives a cleaned key
 * value; other locales remain empty for translation.
 *
...
import { build_sql_cli_command, get_connection_string } from "./db";
import { BOLD, color, confirm, CYAN, dim, GREEN, header, log_action, press_enter, YELLOW } from "./ui";
 ...
 * Generates one row per active language. The default English locale receives
 * the cleaned key name; all other locales receive an empty translation value.
 *
...
export function write_missing_sql(missing: MissingKey[], output_dir: string): SqlFileResult {
	...
	const sql_path = join(output_dir, `insert_translations_${timestamp}.sql`);

	const total_rows = missing.length * locales.length;
	...
	const sql_lines: string[] = [
		...
		`-- (one per active language: ${locales.join(", ")})`,
		`-- The ${default_locale} value is derived from each key. Fill in other locales via the Translations admin UI after importing.`,
		"",
	...
	for (const { namespace, key_path } of missing) {
		...
		const english_value = cleaned_english_value(key_path).replace(/'/g, "''");
		for (const locale of locales) {
			const translation = locale === default_locale ? english_value : "";
			values.push(`('${locale}', '${ns_escaped}', '${kp_escaped}', '${translation}')`);
		}
	}
...
export async function insert_translations(): Promise<void> {
	...
	// Write .sql file
	const total_rows = result.missing.length * locales.length;
	const { path: sql_path } = write_missing_sql(result.missing, cwd);
	const rel_path = relative(cwd, sql_path);

	console.log(`  ${color("✓", GREEN)} Wrote ${total_rows} INSERT row(s) across ${locales.length} language(s) to:`);
	...
	if (conn_str) {
		const sql_cli_cmd = build_sql_cli_command(conn_str, rel_path);
		console.log(`    ${color(BOLD + (process.platform === "win32" ? sql_cli_cmd.ps1 : sql_cli_cmd.sh), CYAN)}`);
	}
	console.log();
	console.log(`  ${dim("After importing, fill in translation values via the Translations admin UI.")}`);

	log_action(`Wrote insert-translations SQL: ${rel_path}`);
