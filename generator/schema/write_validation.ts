...
export async function write_validation_file(
	...
	for (const col of schema_obj.columns) {
		if ((MAINTENANCE_FIELDS as readonly string[]).includes(col.name.toLowerCase())) {
			maintenance_fields[col.name] = {
				name: col.name,
				// See field_generator.ts's identical cast comment - to_html_input()
				// always returns a FieldKind member by construction.
				type: type_mapper.to_html_input(col.type_string) as FieldKind,
				required: false,
				is_nullable: false,
				min: undefined,
				max: undefined,
