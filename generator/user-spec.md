...
- Follow existing generator patterns (see `generator/crud/main.ts`, `generator/add_locale.ts`)
...
The `users` table structure (from `sql/mysql/init/01-init-mysql.sql` / `sql/sqlite/init/01-init-sqlite.sql`):
...
Use the `--dev` flag and a dedicated test database (SQLite in a temp file) via `bun test` + `mock.module("$config/db")` to inject a test connection. The project's existing init SQL (`sql/sqlite/init/01-init-sqlite.sql`) provides the `users` table DDL.
...
2. Runs the `users` table DDL from `sql/sqlite/init/01-init-sqlite.sql` against it
