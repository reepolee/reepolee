...
1. **Split remaining oversized generator and admin modules.** The largest production files are `scripts/mcp/index.ts`, `generator/ddl_cache.ts`, `queue/index.ts`, `generator/crud/main.ts`, and `lib/middleware/rate_limit.ts`. Server lifecycle helpers and DDL-cache file storage are already extracted. Keep future splits focused on natural ownership boundaries, especially DDL cache parsing and MCP tool registration.
