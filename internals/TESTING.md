...
needs. These tests import the module under test via the `$root`/`$lib`/`$config` aliases
(e.g. `$root/generator/naming`) rather than relative paths, since they no longer sit next
to their source.
...
bun run test:dev              # Dev/tooling tests only (tests-dev/)
bun run test:user             # User-facing tests only (lib/, apps/main/, platform/, config/, components/, integration.test.ts)
...
`test:user` and `test:dev` pass their paths with a leading `./`. Bun treats positional
...
- **Location:** Tests sit next to their source files (`lib/*.test.ts` next to `lib/*.ts`). This keeps imports simple (`./helpers` instead of `../../lib/helpers`). The one exception is `tests-dev/` (see above).
...
- **Generator tests** live in `tests-dev/generator/crud/` (e.g., `output.test.ts`, `schema_reader.test.ts`) - dev-only, not shipped.
- **Server tests** live at `apps/main/server.test.ts`, beside the server they cover - user-facing, shipped.
- **Module tests** ship with the module that owns them and are excluded from the distribution via `.releaseignore`.
...
```typescript
import { mock_db, mock_auth_middleware, mock_req, with_temp_dir, setup_template_mocks } from "$root/test_helpers";
```
...
| `setup_template_mocks()`   | Convenience: calls `mock.module("$config/db", mock_db)` + `mock.module("$platform/auth/middleware", mock_auth_middleware)`.                  |
...
- **Avoid DB connections:** Set `DEV_CONNECTION_STRING=''` in unit tests that don't need a real database.
...
`apps/main/server.test.ts` mocks 10+ config/service modules to test the server module without booting
a full HTTP server (avoids PID files, S3 checks, Redis connections):

| Module                        | Purpose                |
| ----------------------------- | ---------------------- |
| `$config/supported_locales`   | Locale configuration   |
| `$config/db`                  | DB access (empty mock) |
| `$main/routes`                | Route definitions      |
...
| `$config/supported_locales`           | **MEDIUM** - similar configs, may differ in `locale_names` keys.   |
| `$config/db_structure`                | **LOW** - similar structure constants. |
| `$platform/auth/middleware` | **LOW** - all use shared `mock_auth_middleware()`. |
| `$platform/auth/cookies`    | **LOW** - same `get_session_id_from_request` pattern. |
...
9. **Use `DEV_CONNECTION_STRING=''`** when running tests that must not hit the database.
