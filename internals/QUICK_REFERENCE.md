...
# Command line
bun reeman crud <table>                              # Full pipeline (schema + CRUD)
bun reeman schema <table>                            # Schema only
bun reeman crud <table> --pagination cursor          # Custom pagination
bun reeman refresh-crud <table> --mode fields        # Update after DDL changes
```

### Marketplace Installation

```bash
bun reeman install <archive.tar.gz>
```

The archive must contain one top-level folder with `install.ps1` and `install.sh`. Reeman warns you to back up the repository and database, asks before installing, runs the script for the current platform, then asks whether to keep the unpacked `marketplace/<folder>/` directory.
...
# Command line
bun reeman remove-route <route_path>
```
...
Run SQL against the app's DB via `lib/sql_runner`; prints per-statement `{ meta, records }` JSON.
Read-only by default (single SELECT per statement, records capped); `--allow-changes` permits writes/DDL.

```bash
bun run sql <file.sql>                     # Run a SQL file
printf 'select * from users;' | bun run sql   # Reads stdin when no file is given
bun run sql <file.sql> --allow-changes     # Opt into writes/DDL
bun run sql <file.sql> --limit=50          # Cap records per statement (default 100, max 1000)
```
...
| File | Zone | Safe? | How |
|------|------|-------|-----|
| **form.ree** | Inside `<!-- GEN:FIELDS:START/END -->` | ✅ | Edit HTML, add CSS, add event listeners |
| **index.ree** | Inside `<!-- GEN:FIELDS:*:START/END -->` | ✅ | Edit headers, cells, add styling |
...
```
Good for: < 100k rows
Generated with: bun reeman crud <table>
...
Generated with: bun reeman crud <table> --pagination cursor
...
```bash
# 1. Generate parent
bun reeman crud orders

# 2. Generate child with parent reference
bun reeman crud order_items --parent orders
...
| Problem | Quick Fix |
|---------|-----------|
| Column not appearing in form | Run `bun reeman refresh-crud <table> --mode fields` |
| Marker not found error | Check markers exist, don't delete `<!-- GEN:FIELDS:START/END -->` |
