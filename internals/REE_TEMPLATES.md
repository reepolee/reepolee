...
- **ReeTags** (`components/*.ree`) - server-rendered, compiled by the `.ree` engine at request time from `props.attributes`/`props.children`. Any tag name containing a hyphen is auto-routed to `$components/<tag-name>.ree` (see `internals/REE_TEMPLATES.md` "Component includes"). This is what `tags` mode targets.
...
| `input-yes-no.ree`             | boolean `number` fields    | Hardcoded 0/1 `<select>`, labels via `props.selectors["0"]`/`["1"]`   |
...
The generator's tag-mode wrapper templates live in `generator/templates/fields_tags/*.ree` (one thin file per field type, mirroring `generator/templates/fields/*.ree`'s flat-mode set) and are substituted the same way via `apply_template()` - no new templating mechanism, just a different template directory selected by `generate_input_field()` (`generator/crud/form_ree.ts`).
