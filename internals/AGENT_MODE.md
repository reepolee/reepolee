...
> `platform/auth/middleware.ts` before relying on details here.
...
curl http://localhost:2500/users -H "X-Agent-User-Username: admin"
```
...
| `platform/auth/middleware.ts` | `resolve_session()` resolves user via header/env var, returns fake session              |
