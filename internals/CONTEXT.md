...
- **`extract_untranslated()` (`lib/translation_merge.ts`)** - Flags only `::missing::`-prefixed locale values for AI translation. The prefix is the source of truth, so intentionally English values are not re-sent.
...
- **`db:clone-test`** - Script at `scripts/clone_db.ts`. Regenerates the test DB (from `TEST_CONNECTION_STRING`) from a temporary development DB dump (from `DEV_CONNECTION_STRING`). Supports `--yes` (skip confirmation), `--dry-run`, and `--no-data` (DDL only, no row data). It uses the same native artifacts as `scripts/dump_db.ts`: MySQL runs `mysqldump` and restores `dump.sql` with `mysql`, while SQLite copies the database file. The target is replaced so stale tables and SQLite sidecar state cannot survive. Snapshot/restore modes use the same dump and restore path.
