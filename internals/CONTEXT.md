...
- **`ctx.translations`** - Populated inside `create_ctx()`, one resolution per request, using alias-resolved `ctx.ui_locale` and `ctx.route_dir`. Namespace and root fallback merge in `create_ctx()` / `RequestContext`. Translation files are loaded into memory once at startup and refreshed by `POST /__reload-translations` after file edits.
