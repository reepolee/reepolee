...
Translations are DB-backed (the `translations` table), but the ree-templates VS Code extension shows *ghost values* next to `{_ ... }` tags in `.ree` files by reading co-located JSON. `lib/emit_translations.ts` bridges the two: on `bun dev` bootstrap and on `/__reload-translations` (dev only, never production), it dumps the full per-locale trees to `.reepolee/i18n/<locale>.json`. These files are a working-folder aid, **not** a source of truth - the DB is. This mirrors how ree-web reads its co-located locale JSONs.
...
Called from every generated index route (`generator/templates/index/*.ts`) and from the hand-written system routes that use scopes (`routes/system/users/index.ts`, `routes/system/modules/handlers.ts`, `routes/system/global_scopes/index.ts`).
...
generation in the session. reeman warns and names the repair command; the server
fails loud (`lib/verify_locale_tables.ts`), because a drifted clone silently
serves wrong data for one locale.

## Translations (i18n)

- **`translations` table** - The single source of truth for all translation values. Schema: `(locale, namespace, key_path, translation)`, with a full BCP 47 locale such as `en-us`. Inserted/updated by `bun reeman sync-translations` (AI-powered) and the `/system/translations` admin UI.
...
- **`sync-translations`** - Scans the DB for namespaces, translates missing keys via the configured AI provider (Ollama, Gemini, Hugging Face, or OpenRouter), and writes results back to the DB.
...
- **`insert-translations`** - Scans `.ree` templates for referenced keys missing from the DB and writes a reviewable INSERT SQL file; it does not execute the SQL automatically.
- **`add_translations`** - Inserts explicitly supplied `(locale, namespace, key_path, translation)` records, skipping records that already exist. Reeman exposes the same operation as the interactive "Add translation" action.
...
- **`ctx.translations`** - Populated inside `create_ctx()`, one resolution per request, using alias-resolved `ctx.ui_locale` and `ctx.route_dir` (both already resolved earlier in the same function). The merge logic (namespace + `root` fallback, `_merged_translations_cache`) lives in `create_ctx()` / `RequestContext`. Handlers read it directly for their own logic (`ctx.translations.errors`, validation-message lookups, `format_bulk_delete_message`); `render()` reads the same `ctx.translations` internally to build template data - no separate `translations` key needs to be passed through `data` at all. One resolution, one place it lives, matching how `ctx.user` / `ctx.locale` / `ctx.toasts` already work. Translations are loaded into memory once at server startup via `TranslationRepository`, not re-fetched from the DB per request - `reload()` only runs on the `POST /__reload-translations` webhook. This is a pre-release API - `create_ctx()`'s signature and `RequestContext`'s shape are free to change without back-compat concerns.
...
- **Translations are mandatory infrastructure** - Every app built on this framework goes through the DB-backed `translations` table for UI strings, even a single-locale deployment (`config/supported_locales.ts` with `locales = ["en-us"]` only). There is no simpler path where a route hand-writes source-locale strings directly in `.ree` files instead of a `key_path` lookup - multi-locale readiness is the default posture from day one, not an opt-in layered on later. In the finished route, `props.translations` is always expected to be present. During active development, though, it is common and legitimate to scaffold a route's layout and business data first and wire up translation keys afterward - `props.translations` genuinely does not exist yet during that window, and that is not a bug to fail loudly on.
...
generator, and no `routes/api/registry.ts` exist in the codebase - the design
