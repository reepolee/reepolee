...
translation reloads, and CRUD regeneration are not exposed until the local
operator starts MCP with `MCP_ENABLE_MUTATIONS=true`.
...
| `run_generator`       | Invoke a code generator (call `list_generators` for the current set - includes `sync_locale_tables`, which reconciles the per-locale clone tables); only with mutation capability |
...
| `insert_translations` | Scan .ree templates for missing DB keys; optionally write a reviewable INSERT SQL file |
| `add_translations`  | Insert explicitly supplied `{locale, namespace, key_path, translation}` records; existing records are skipped |
| `prune_translations` | Find DB translation keys no longer referenced in templates; optionally write DELETE SQL |
...
All translation-writing and translation-synchronization operations require
`MCP_ENABLE_MUTATIONS=true`. `insert_translations` and `prune_translations` only
write SQL files when `write_sql` is true; they never execute those files. After
`add_translations` or `sync_translations`, the server translation maps are
