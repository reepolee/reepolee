...
- **`bun devw`** runs the same via `scripts/dev_run.ts --worker`, additionally starting `bun --hot worker.ts` (also auto-enabled whenever `REDIS_URL` is set). Use `bun run worker` to start the worker on its own instead.
