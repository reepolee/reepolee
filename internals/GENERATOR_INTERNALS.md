...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `schema <table\|all>`       | `[--prefix <dir>] [--parent <table>]` - introspects DB, writes `routes/<table>/schema/`. `--parent` detects FK and nests child under `routes/<parent>/<child>/`.                                                          |
...
| locale | namespace | key_path | translation |
|------|-----------|----------|-------------|
| en-us | home | labels.modules_tags.admin | Admin |
...
The generator leaves that slot as an inert HTML comment, `<!-- hints.__field.name__ -->`, so a fresh generation renders no hint by default. To activate it for a field, replace the comment with `{_ hints.<field_name>}` and add the corresponding `hints.<field_name>` key to the route's translation namespace. `routes/system/global_scopes/form.ree` is the reference example - every field there has a populated hint. Because this is inside the `<!-- crud:fields:start/end -->` managed section, hand-added hints are preserved by `bun reeman refresh-crud <table> --mode fields` as long as the field-wrapper's structure is otherwise unchanged (see [CRUD refresh (field-only)](CONTEXT.md#crud-refresh-field-only)).
