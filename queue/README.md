...
```json
{
	...
	"devw": "bun scripts/dev_run.ts --worker",
	"worker": "bun worker.ts"
}
```

`scripts/dev_run.ts` is a small Bun-native orchestrator: it always builds CSS and starts `bun --hot server.ts --dev`, and also starts `bun --hot worker.ts` when either `--worker` is passed or `REDIS_URL` is set. Output from each child is streamed with a `[dev]`/`[wk]` prefix, and SIGINT/SIGTERM are forwarded to both - no external process manager needed.
