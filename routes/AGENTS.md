...
- Each route module exports `export const route_definitions: RouteDefinition[]` (type from `$lib/route_builder`). `routes/routes.ts` aggregates every module's `route_definitions`, including explicitly mounted development modules outside `routes/`, and builds the table via `build_routes()` / `build_nav_routes()`.
