...
async function post_system_queues_clear(req: BunRequest): Promise<Response> {
	...
	if (!queue) { return Response.redirect(localized_url("/system/queues", locale), 303); }
	...
	return Response.redirect(localized_url("/system/queues", locale), 303);
}
...
async function post_system_queues_retry(req: BunRequest): Promise<Response> {
	...
	if (!job_id) { return Response.redirect(localized_url("/system/queues", locale), 303); }
