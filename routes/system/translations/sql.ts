...
export async function get_key_by_id(id: string): Promise<{ namespace: string; key_path: string; values: Record<string, string>; } | undefined> {
	return await timed_query("translations", "get_key_by_id", async () => {
		...
		const rows = (await db`SELECT locale, translation FROM translations WHERE namespace = ${namespace} AND key_path = ${key_path}`) as { locale: string; translation: string; }[];
		if (rows.length === 0) return undefined;

		const values: Record<string, string> = {};
		for (const row of rows) {
			values[row.locale] = row.translation;
		}
...
export async function delete_key(namespace: string, key_path: string): Promise<number> {
	return await timed_query("translations", "delete_key", async () => {
		...
		return result.affectedRows ?? result.count ?? result.changes ?? 0;
	});
}
...
export async function delete_translation(locale: string, namespace: string, key_path: string): Promise<number> {
	return await timed_query("translations", "delete_translation", async () => {
		const result = await db`DELETE FROM translations WHERE locale = ${locale} AND namespace = ${namespace} AND key_path = ${key_path}`;
		return result.affectedRows ?? result.count ?? result.changes ?? 0;
	});
}
...
async function upsert_translation_sqlite(locale: string, namespace: string, key_path: string, translation: string): Promise<void> {
	await db`
		INSERT INTO translations (locale, namespace, key_path, translation)
		VALUES (${locale}, ${namespace}, ${key_path}, ${translation})
		ON CONFLICT(locale, namespace, key_path) DO UPDATE SET translation = excluded.translation
	`;
}

async function upsert_translation_mysql(locale: string, namespace: string, key_path: string, translation: string): Promise<void> {
	await db`
		INSERT INTO translations (locale, namespace, key_path, translation)
		VALUES (${locale}, ${namespace}, ${key_path}, ${translation})
...
export async function upsert_translation(locale: string, namespace: string, key_path: string, translation: string): Promise<void> {
	return await timed_query("translations", "upsert", async () => {
		if (db_type === "sqlite") { return upsert_translation_sqlite(locale, namespace, key_path, translation); }
		return upsert_translation_mysql(locale, namespace, key_path, translation);
	});
}
...
export async function get_all_locales(): Promise<string[]> {
	return await timed_query("translations", "get_all_locales", async () => {
		const rows = (await db`SELECT DISTINCT locale FROM translations ORDER BY locale`) as { locale: string; }[];
...
export async function get_all_translations(namespace_filter: string = ""): Promise<TranslationRow[]> {
	return await timed_query("translations", "get_all_translations", async () => {
		...
		if (namespace_filter) {
			rows = (await db`SELECT * FROM translations WHERE namespace = ${namespace_filter} ORDER BY namespace, key_path, locale`) as TranslationRow[];
		} else {
			rows = (await db`SELECT * FROM translations ORDER BY namespace, key_path, locale`) as TranslationRow[];
		}
		return rows;
	});
}
 ...
 *
 * sort_field is regex-validated against an allow-list before interpolation.
 * "parent_path" isn't a real column (it's derived from key_path in JS), so it
 * sorts by key_path instead - a close approximation since parent_path is a
 * prefix of key_path.
 */
export async function get_translations_page(
	...
	return await timed_query("translations", "get_page", async () => {
		...
		const order_column = sort_field === "parent_path" ? "key_path" : "namespace";
		const order_dir = sort_dir === "desc" ? "DESC" : "ASC";
		...
		const distinct_keys = (await db.unsafe(`
			...
			${where_clause}
			ORDER BY ${order_column} ${order_dir}, namespace, key_path
		...
		return (await db.unsafe(`
			...
			ORDER BY ${order_column} ${order_dir}, namespace, key_path, locale
		`)) as TranslationRow[];
	});
}
	...
	return await timed_query("translations", "delete_groups", async () => {
		...
		for (const g of groups) {
			if (g.parent_path) {
				...
				total += result.affectedRows ?? result.count ?? result.changes ?? 0;
			} else {
