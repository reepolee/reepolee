...
export async function get_all_records(): Promise<Record[]> {
	try {
		const records = await db`SELECT * FROM files ORDER BY id ASC`;
...
export async function get_files_select_options(): Promise<Options[]> {
	try {
		const records = await db`SELECT id as option_value, folder as option_text FROM files ORDER BY folder ASC`;
...
export async function create_record(record: Omit<Record, "id">): Promise<Record> {
	try {
		const insert_result = await db`INSERT INTO files (folder, filename, s3_key, original_filename, title, description, tags, mime_type, file_type, file_size) VALUES (${record.folder}, ${record.filename}, ${record.s3_key}, ${record.original_filename}, ${record.title}, ${record.description}, ${record.tags}, ${record.mime_type}, ${record.file_type}, ${record.file_size})`;
