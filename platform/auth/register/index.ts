 ...
 * [0]          [1]            [2]
 *
 * Segments are decoded defensively: a malformed percent-escape (e.g. %zz)
 * makes decodeURIComponent throw URIError, and throwing here would surface as
 * an unhandled rejection with no response. Malformed segments decode to "" so
 * the caller's invalid-link handling takes over.
 * Exported for tests.
 */
export function extract_register_params(req: BunRequest): { username: string; invitation_code: string; } {
	const parts = new URL(
		req.url,
		"http://localhost",
	).pathname.split("/").filter(Boolean);
	const safe_decode = (value: string): string => {
		try { return decodeURIComponent(value); } catch { return ""; }
	};
	return {
		username: safe_decode(parts[1] || "").toLowerCase(),
		invitation_code: safe_decode(parts[2] || ""),
	};
}
