...
import type { User_record } from "./types";
...
/**
 * Create a new session for a verified user and return the session Cookie.
 * Coerces all nullable DB fields to empty strings so the session store and
 * templates never receive null.
 */
export async function create_user_session(user: User_record, req: BunRequest): Promise<Cookie> {
	...
	return build_session_cookie(session_id, req);
}
