/**
 * Session store backed by SQLite via Bun native SQL.
 * All public functions follow a KV contract so the backing store can be
 * swapped to Redis / Valkey with zero changes at call-sites.
 */
...
async function destroy_user_sessions(user_id: number): Promise<void> {
	await db`DELETE FROM sessions WHERE CAST(json_extract(session_json, '$.user_id') AS INTEGER) = ${user_id}`;
}
...
async function cleanup_expired(): Promise<number> {
	...
	const result = await db`DELETE FROM sessions WHERE CAST(json_extract(session_json, '$.created_at') AS INTEGER) < ${cutoff}`;
	return Number((result as any)?.count ?? (result as any)?.changes ?? 0);
}
...
console.log("\u001b[34mUsing Session store SQLITE\u001b[0m");
