...
async function get_live_authz(user_id: number): Promise<Authz_entry> {
	...
	// Expired entries are evicted on read so the Map cannot grow unbounded on
	// a long-lived server: without this, every user_id ever authenticated would
	// accumulate a permanent entry even after its 5 s TTL passed.
	if (cached && cached.expires_at <= now) { _authz_cache.delete(user_id); }
 ...
 * users CRUD (update_record / archive_record in apps/reeman/users/sql.ts).
...
const AGENT_SECRET = env_available("AGENT_SECRET") ? Bun.env.AGENT_SECRET : undefined;
...
export function require_auth(auth_ctx: Auth_context, req?: BunRequest): Response | null {
	...
	const locale = req ? resolve_locale(req) : default_locale;
	let login_url = localized_url("/login", locale);
