...
if (!redis_available()) {
	console.error("\u001b[31m[session] Redis unavailable - cannot use Redis session store.\u001b[0m");
	console.error("\u001b[31m[session] Set REDIS_ENABLED=true and REDIS_URL to a real redis:// URL, or change SESSION_STORE to \"sql\".\u001b[0m");
	throw new Error("SESSION_STORE=redis requires Redis to be enabled and REDIS_URL to be set");
}

const redis = new RedisClient(Bun.env.REDIS_URL!.trim());
