...
import { acquire_users_table_lock, make_test_db_mock, get_test_db_connection } from "$root/test_helpers";

const DB_TIMEOUT_MS = 5_000;

// Every row this file writes uses this email prefix so clean_db() only ever
// deletes rows this file owns. sql.test.ts, integration.test.ts, and
// tests-dev/generator/user.test.ts all hit the same real TEST_CONNECTION_STRING
// database from separate `bun test --parallel` worker processes. This file
// and integration.test.ts hold acquire_users_table_lock() for their whole
// run so user.test.ts's full-table DELETE (needed for its "table is empty"
// assertions) can never interleave with these prefix-scoped inserts.
const EMAIL_PREFIX = "sql_test__";

let test_db: SQL | null = null;

try {
	test_db = await Promise.race([
		Promise.resolve(get_test_db_connection()),
		new Promise<SQL>((_, reject) => setTimeout(() => reject(new Error(`Database connection timed out after ${DB_TIMEOUT_MS}ms`)), DB_TIMEOUT_MS)),
	]);
...
if (test_db) {
	const release_users_table_lock = await acquire_users_table_lock();
	afterAll(release_users_table_lock);
}
...
// Clean database before each test - scoped to this file's own rows (see EMAIL_PREFIX).
async function clean_db() { await test_db!.unsafe("DELETE FROM users WHERE email LIKE ?", [`${EMAIL_PREFIX}%`]); }

beforeEach(async () => await clean_db());

run("auth/sql.get_user_by_email", () => {
	test("returns user when email exists", async () => {
		await test_db!`INSERT INTO users (email, username, created_at) VALUES (${"sql_test__test@example.com"}, ${"testuser"}, ${"2026-01-01 00:00:00"})`;

		const user = await get_user_by_email("sql_test__test@example.com");
		expect(user).toBeDefined();
		expect(user?.email).toBe("sql_test__test@example.com");
...
run("auth/sql.get_user_by_username", () => {
	test("returns user when username exists", async () => {
		await test_db!`INSERT INTO users (email, username, created_at) VALUES (${"sql_test__john@example.com"}, ${"johndoe"}, ${"2026-01-01 00:00:00"})`;

		const user = await get_user_by_username("johndoe");
		expect(user).toBeDefined();
		expect(user?.username).toBe("johndoe");
		expect(user?.email).toBe("sql_test__john@example.com");
	});
...
run("auth/sql.get_user_by_id", () => {
	test("returns user when ID exists", async () => {
		await test_db!`INSERT INTO users (email, username, created_at) VALUES (${"sql_test__alice@example.com"}, ${"alice"}, ${"2026-01-01 00:00:00"})`;
		const rows = await test_db!`SELECT id FROM users WHERE email = ${"sql_test__alice@example.com"} LIMIT 1`;
		const user_id = (rows[0] as any).id;

		const user = await get_user_by_id(user_id);
		expect(user).toBeDefined();
		expect(user?.email).toBe("sql_test__alice@example.com");
	});
...
run("auth/sql.get_user_by_invitation_code", () => {
	test("returns user when invitation code matches", async () => {
		...
		await test_db!`INSERT INTO users (email, username, invitation_code, created_at) VALUES (${"sql_test__invited@example.com"}, ${"invited_user"}, ${code}, ${"2026-01-01 00:00:00"})`;

		const user = await get_user_by_invitation_code(code);
		expect(user).toBeDefined();
		expect(user?.invitation_code).toBe(code);
		expect(user?.email).toBe("sql_test__invited@example.com");
	});
...
run("auth/sql.verify_and_register_user", () => test("updates user with name, password, and verified_at", async () => {
	await test_db!`INSERT INTO users (email, username, created_at) VALUES (${"sql_test__newuser@example.com"}, ${"newuser"}, ${"2026-01-01 00:00:00"})`;
	const rows = await test_db!`SELECT id FROM users WHERE email = ${"sql_test__newuser@example.com"} LIMIT 1`;
	const user_id = (rows[0] as any).id;
...
run("auth/sql.create_invited_user", () => test("inserts user and returns the created user", async () => {
	...
	const user = await create_invited_user("sql_test__guest@example.com", "guest_user", code);

	expect(user).toBeDefined();
	expect(user?.email).toBe("sql_test__guest@example.com");
...
run("auth/sql.update_user_profile", () => {
	test("updates user name and nickname, preserves avatar", async () => {
		await test_db!`INSERT INTO users (email, username, name, nickname, avatar_filename, created_at) VALUES (${"sql_test__profile@example.com"}, ${"profile_user"}, ${"Old Name"}, ${"old_nick"}, ${"avatar.jpg"}, ${"2026-01-01 00:00:00"})`;
		const rows = await test_db!`SELECT id FROM users WHERE email = ${"sql_test__profile@example.com"} LIMIT 1`;
		const user_id = (rows[0] as any).id;
	...
	test("updates avatar_filename when provided", async () => {
		await test_db!`INSERT INTO users (email, username, avatar_filename, created_at) VALUES (${"sql_test__avatar@example.com"}, ${"avatar_user"}, ${"old.jpg"}, ${"2026-01-01 00:00:00"})`;
		const rows = await test_db!`SELECT id FROM users WHERE email = ${"sql_test__avatar@example.com"} LIMIT 1`;
		const user_id = (rows[0] as any).id;
...
run("auth/sql.update_user_password", () => {
	test("updates hashed_password and previous_hashed_password", async () => {
		await test_db!`INSERT INTO users (email, username, hashed_password, created_at) VALUES (${"sql_test__pwd@example.com"}, ${"pwd_user"}, ${"old_hash"}, ${"2026-01-01 00:00:00"})`;
		const rows = await test_db!`SELECT id FROM users WHERE email = ${"sql_test__pwd@example.com"} LIMIT 1`;
		const user_id = (rows[0] as any).id;
