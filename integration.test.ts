...
import { acquire_users_table_lock, get_test_db_connection } from "$root/test_helpers";

// Every row this file writes uses this email prefix so clean_db() only ever
// deletes rows this file owns. This file, platform/auth/sql.test.ts, and
...
// and sql.test.ts hold acquire_users_table_lock() for their whole run so
// user.test.ts's full-table DELETE (needed for its "table is empty"
// assertions) can never interleave with these prefix-scoped inserts.
const EMAIL_PREFIX = "integration_test__";
...
async function clean_db() {
	const db = await get_test_db_connection();
	try {
		await db.unsafe("DELETE FROM users WHERE email LIKE ?", [`${EMAIL_PREFIX}%`]);
	} finally {
		await db.close();
	}
}
...
describe("Auth Middleware Integration with DB", () => {
	test("resolve_session returns null user when no session", async () => {
		const { resolve_session } = await import("$platform/auth/middleware");
	...
	test("require_auth guard redirects unauthenticated users", async () => {
		const { require_auth } = await import("$platform/auth/middleware");
...
// Note: Language middleware tests are covered in unit tests (lib/middleware/set_locale.test.ts)
...
describe("Module Authorization Middleware Integration", () => {
	test("require_module_mw allows users with required module", async () => {
		...
		const { require_module } = await import("$platform/auth/middleware");
...
describe("Auth SQL Integration with Real DB", () => {
	test("can insert user into database without error", async () => {
		...
		await expect(create_test_user(`${EMAIL_PREFIX}alice@example.com`, "alice")).resolves.toBeUndefined();
	});

	test("auth functions handle real database gracefully", async () => {
		const { get_user_by_email } = await import("$platform/auth/sql");

		// Should return undefined for non-existent user
		const user = await get_user_by_email(`${EMAIL_PREFIX}nonexistent@example.com`);
		expect(user).toBeUndefined();
	});

	test("password update function works with database", async () => {
		const { update_user_password } = await import("$platform/auth/sql");
