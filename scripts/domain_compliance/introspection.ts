...
function extract_expr_default(create_sql: string, column: string): string | null {
	...
	// Scan forward from the opening paren to its balanced closing paren - the
	// expression can itself contain parentheses, so a plain capture group
	// would stop at the first ")".
	const start = match.index + match[0].length;
	let depth = 1;
	for (let i = start; i < create_sql.length; i++) {
		const char = create_sql[i];
		if (char === "(") { depth++; } else if (char === ")") { depth--; }
		if (depth === 0) { return create_sql.slice(start, i).trim(); }
	}
	return null;
}
