...
// Installers return a short detail string ("8.18.4") that
// becomes the trailing note on the reported line.
async function run_tool(label: string, tool: global_tool_name | null, fn: () => Promise<string>): Promise<void> {
	step_start(label);
	try {
		const detail = await fn();
		if (tool && detail === "added") await record_global_tool(tool);
...
async function main() {
	...
	await run_tool("zod types", null, install_zod_types);

	section("Global tools");

	await run_tool("tailwindcss", "tailwindcss", () => install_tailwind({ version: tw_version, get_task: "get:tw" }));
