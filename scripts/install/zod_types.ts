...
export async function install_zod_types(): Promise<string> {
	...
	try {
		...
		// cwd (not -C <path>) avoids passing a Windows-style path as a tar argument,
		// which some tar builds on Windows misparse as a remote host spec.
		await run("tar", ["-xzf", "zod.tgz"], { cwd: tmp_dir });
