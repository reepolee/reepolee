...
try {
	const rows = await db_cli.unsafe(`
		SELECT locale, namespace, key_path, translation
		FROM translations
		WHERE key_path IN ('username_or_email','username_required','username_exists','invalid_username_or_password','username')
		ORDER BY namespace, key_path, locale
	`);
	for (const r of rows) {
		console.log(`${r.locale.padEnd(4)} ${r.namespace.padEnd(28)} ${r.key_path.padEnd(32)} ${r.translation}`);
	}
