...
const TS_GLOB = new Bun.Glob("**/*.ts");

// Bun.Glob has no negative-pattern support, so SKIP_DIRS is applied per path
// segment afterwards - the same test the recursive walk applied at every level.
function collect_files(dir: string, out: string[] = []): string[] {
	for (const rel of TS_GLOB.scanSync({ cwd: dir })) {
...
async function scan(path: string): Promise<Finding[]> {
	...
	for (let i = 0; i < clean.length; i++) {
		...
		const entering_class = /\bclass\s+[A-Za-z_$][\w$]*/.test(line);
		depth += opens - closes;
