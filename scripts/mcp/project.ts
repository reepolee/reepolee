...
export async function list_translation_namespaces(): Promise<Record<string, string[]>> {
	...
	try {
		const rows = await db`SELECT DISTINCT locale, namespace FROM translations ORDER BY locale, namespace` as { locale: string; namespace: string; }[];

		for (const row of rows) {
			const locale = row.locale;
			const ns = row.namespace || "root";
			if (!ns_by_locale[locale]) ns_by_locale[locale] = [];
			if (!ns_by_locale[locale].includes(ns)) { ns_by_locale[locale].push(ns); }
		}
	} catch {
