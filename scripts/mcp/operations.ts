...
import { find_missing_keys, write_missing_sql } from "$generator/reeman/insert_translations";
...
export async function run_generator(name: string, args: string[] = []): Promise<{ success: boolean; stdout: string; stderr: string; }> {
	...
	// Generators that read the DB schema need a live one. The locale/user/translation
 ...
 */
export async function insert_translations(write_sql = false): Promise<{ missing: any[]; stats: any; sql_path: string | null; }> {
	if (write_sql) { assert_mcp_mutation_enabled(); }
	const cwd = process.cwd();
	const routes_dir = join(cwd, "routes");
	const public_dir = join(cwd, "public");
...
export async function sync_translations(translate = false): Promise<{ namespaces: number; translate: boolean; }> {
	...
	if (translate) {
		...
		const rows = (await db_cli`SELECT COUNT(DISTINCT namespace) AS count FROM translations`) as { count: number; }[];
		return { namespaces: Number(rows[0]?.count ?? 0), translate };
	}

	const rows = (await db_cli`SELECT DISTINCT namespace FROM translations ORDER BY namespace`) as { namespace: string; }[];
	await Promise.all(rows.map((row) => sync_single_namespace(row.namespace, false)));
	await notify_server_reload();
	return { namespaces: rows.length, translate };
}
...
export async function add_translations(entries: TranslationEntry[]): Promise<{ inserted: number; skipped: number; }> {
	...
	for (const entry of entries) {
		const existing = await db`SELECT 1 FROM translations WHERE locale = ${entry.locale} AND namespace = ${entry.namespace} AND key_path = ${entry.key_path} LIMIT 1`;
		if (existing.length > 0) {
			skipped++;
			continue;
		}
		await db`INSERT INTO translations (locale, namespace, key_path, translation) VALUES (${entry.locale}, ${entry.namespace}, ${entry.key_path}, ${entry.translation})`;
		inserted++;
	}
