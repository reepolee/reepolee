 ...
 *
 * Single source of truth for the project root and the directories the MCP
 * tools scan. Imported by index.ts, db.ts, and project.ts so the constants are
 * not re-derived per module. Folder names come from $config/paths.
 */
...
export const ROUTES_DIR = join(PROJECT_ROOT, MAIN_APP);
export const PLATFORM_ROOT = join(PROJECT_ROOT, PLATFORM_DIR);
...
export function resolve_template_file(path: string): string {
	...
	const allowed_roots = [ROUTES_DIR, PLATFORM_ROOT, COMPONENTS_DIR];
	if (!allowed_roots.some((root) => is_within_dir(resolved, root))) {
		throw new Error(`template path: file must be under ${MAIN_APP}, ${PLATFORM_DIR}, or components`);
	}
