 ...
 * - insert_translations      Find translation keys referenced in templates but missing from the DB
...
const tools: Array<{ name: string; description: string; inputSchema: Record<string, any>; handler: ToolHandler; }> = [
	...
	},
	{
		...
		handler: async (args) => {
			...
			if (namespace) {
				const rows = await db`SELECT key_path, translation FROM translations WHERE locale = ${locale} AND namespace = ${namespace} ORDER BY key_path` as {
					key_path: string;
					translation: string;
				}[];

				if (rows.length === 0) { throw new Error(`Translations not found for namespace "${namespace}" in locale "${locale}"`); }
			...
			const ns_rows = await db`SELECT DISTINCT namespace FROM translations WHERE locale = ${locale} ORDER BY namespace` as { namespace: string; }[];

			if (ns_rows.length === 0) { throw new Error(`No translations found for locale "${locale}"`); }

			const result: Record<string, any> = {};
			for (const { namespace: ns } of ns_rows) {
				const key_rows = await db`SELECT key_path, translation FROM translations WHERE locale = ${locale} AND namespace = ${ns} ORDER BY key_path` as {
					key_path: string;
					translation: string;
				}[];
	...
	{
		name: "run_generator",
		description: "Run a code generator. Available: resource (single-table full pipeline), schema, crud, bulk (many tables), nested (child tables under --parent), sync_translations, add_locale, remove_locale, sync_locale_tables (reconcile per-locale clone tables), user, validation.",
		inputSchema: {
			type: "object",
			properties: {
				name: {
	...
	},
	{
		...
		handler: async (args) => {
			const result = await insert_translations(args.write_sql === true);
			return json_content(result);
		},
	},
	{
		name: "sync_translations",
		description: "Sync translation data across all database namespaces. Set translate to true to translate missing values with the configured AI provider.",
		inputSchema: {
			type: "object",
			properties: {
