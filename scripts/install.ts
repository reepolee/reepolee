...
/**
 * Non-interactive project bootstrap, run via `postinstall` so it fires
 * automatically after `bun create`/`bun install`. `postinstall` runs on every
 * `bun install`, not just the first, so this is gated by a marker file
 * instead of re-running (and re-seeding) on every dependency install.
 *
