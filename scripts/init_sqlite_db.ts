 ...
 * DEV_CONNECTION_STRING (freshly read from .env) is SQLite, and only runs core
 * DDL (no example data) - mirrors the statement-splitting used by
 * generator/reeman/quick_start.ts. 
 */
...
async function read_connection_string_from_env(): Promise<string | null> {
	...
	for (const line of lines) {
		...
		const match = trimmed.match(/^(?:export\s+)?DEV_CONNECTION_STRING=(?:"([^"]*)"|'([^']*)'|(\S*))\s*$/);
		if (match) {
...
// --quiet is passed by the installer, which reports progress itself. Running
// this script directly keeps the full per-file statement breakdown.
const is_quiet = process.argv.slice(2).includes("--quiet");

const conn_str = await read_connection_string_from_env();

if (!conn_str || conn_str.toLowerCase().startsWith("mysql://")) {
	if (!is_quiet) { console.log("[init-db] DEV_CONNECTION_STRING is not SQLite - skipping DB init"); }
	process.exit(0);
}
...
try {
	for (const file of SQL_FILES) {
		...
		if (!is_quiet) { console.log(`[init-db] ${file}: ${statements.length} statement(s)`); }

		for (const stmt of statements) {
			await db.unsafe(stmt);
		}
	}
	if (!is_quiet) { console.log("[init-db] SQLite database initialized (core DDL)"); }
} finally {
