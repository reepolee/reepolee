...
if (!conn_str || conn_str.toLowerCase().startsWith("mysql://")) {
	if (!is_quiet) { console.log("[init-db] CONNECTION_STRING is not SQLite - skipping DB init"); }
	process.exit(0);
}

const SQL_FILES = ["sql/sqlite/01-init-sqlite.sql", "sql/sqlite/02-init-translations-en-us.sql"];
...
try {
	...
	if (!is_quiet) { console.log("[init-db] SQLite database initialized (DDL + EN translations)"); }
} finally {
