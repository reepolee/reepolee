...
import { cleanup, hard_clone, save_file_upload } from "$lib/file_processor";
...
export async function post_files_save(req: BunRequest): Promise<Response> {
	...
	try {
		...
		// Table-agnostic adapter: validate + save temp + store (no DB write).
		...
		const uploaded_by_user_id = auth_ctx.current_user?.id ?? 0;

		const rows = await db`
			INSERT INTO files (
				folder, filename, s3_key, original_filename, title, description, tags, mime_type, file_type, file_size, uploaded_by_user_id
			)
			VALUES (
				${folder}, ${result_filename}, ${final_s3_key}, ${original_name}, ${title}, ${description}, ${tags}, ${result.mime}, ${file_type}, ${result.file_size}, ${uploaded_by_user_id}
			)
			RETURNING id
		`;
