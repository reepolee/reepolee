...
export async function get_all_records(): Promise<Record[]> {
	try {
		const records = await db`SELECT * FROM files WHERE archived_at IS NULL ORDER BY id ASC`;
...
export async function get_files_select_options(): Promise<Options[]> {
	try {
		const records = await db`SELECT id as option_value, folder as option_text FROM files WHERE archived_at IS NULL ORDER BY folder ASC`;
...
export async function get_record_by_id(id: number, include_archived: boolean = false): Promise<Record | undefined> {
	try {
		...
		const by_id_query = `SELECT * FROM files WHERE id = ?${archive_where} LIMIT 1`;
...
export async function search_records(
	...
	try {
		...
		const query = `SELECT * FROM v_files ${where} ORDER BY ${sort_field} ${valid_direction.toUpperCase()}, id ${valid_direction.toUpperCase()} LIMIT ? OFFSET ?`;
		records = await db.unsafe(query, [...params, limit, offset]);

		const count_query = `SELECT COUNT(*) as count FROM v_files ${where}`;
...
export async function create_record(record: Omit<Record, "id">): Promise<Record> {
	try {
		const insert_result = await db`INSERT INTO files (folder, filename, s3_key, original_filename, title, description, tags, mime_type, file_type, file_size, uploaded_by_user_id) VALUES (${record.folder}, ${record.filename}, ${record.s3_key}, ${record.original_filename}, ${record.title}, ${record.description}, ${record.tags}, ${record.mime_type}, ${record.file_type}, ${record.file_size}, ${record.uploaded_by_user_id})`;
		const get_result = await db`SELECT * FROM files WHERE id = ${insert_result.lastInsertRowid} LIMIT 1`;
...
export async function update_record(id: number, record: Omit<Record, "id">): Promise<Record | undefined> {
	try {
		await db`UPDATE files SET folder = ${record.folder}, filename = ${record.filename}, s3_key = ${record.s3_key}, original_filename = ${record.original_filename}, title = ${record.title}, description = ${record.description}, tags = ${record.tags}, mime_type = ${record.mime_type}, file_type = ${record.file_type}, file_size = ${record.file_size} WHERE id = ${id}`;
		const records = await db`SELECT * FROM files WHERE id = ${id} LIMIT 1`;
...
export async function archive_record(id: number, archived_by_user_id: number | null): Promise<boolean> {
	try {
		const result = await db`UPDATE files SET archived_at = CURRENT_TIMESTAMP, archived_by_user_id = ${archived_by_user_id} WHERE id = ${id} AND archived_at IS NULL`;
...
export async function restore_record(id: number): Promise<boolean> {
	try {
		const result = await db`UPDATE files SET archived_at = NULL, archived_by_user_id = NULL WHERE id = ${id} AND archived_at IS NOT NULL`;
		return (result.affectedRows ?? result.count ?? result.changes ?? 0) > 0;
	} catch (error) {
...
export async function get_archive_counts(scope_clause: string = ""): Promise<ArchiveCounts> {
	try {
		const counts_query = archive_counts_query("files", scope_clause);
