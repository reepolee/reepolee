...
import { invalidate_authz } from "$platform/auth/middleware";
...
/**
 * Record copy with credentials removed, for sql_log calls - password hashes
 * and invitation codes must never be written to the plaintext SQL log file.
 * Accepts undefined (mirrors the old `{ ...record }` spread) and returns {}.
 */
export function strip_log_sensitive(record: Record | undefined): Partial<Record> {
	if (!record) return {};
	const { hashed_password, previous_hashed_password, invitation_code, ...safe } = record;
	return safe;
}
...
export async function get_all_records(): Promise<Record[]> {
	try {
		const records = await db`SELECT * FROM users WHERE archived_at IS NULL ORDER BY id ASC`;
...
export async function get_users_select_options(): Promise<Options[]> {
	try {
		const records = await db`SELECT id as option_value, username as option_text FROM users WHERE archived_at IS NULL ORDER BY username ASC`;
...
export async function get_record_by_id(id: number, include_archived: boolean = false): Promise<Record | undefined> {
	try {
		...
		const by_id_query = `SELECT * FROM users WHERE id = ?${archive_where} LIMIT 1`;
...
export async function archive_record(id: number, archived_by_user_id: number | null): Promise<boolean> {
	try {
		const result = await db`UPDATE users SET archived_at = CURRENT_TIMESTAMP, archived_by_user_id = ${archived_by_user_id} WHERE id = ${id} AND archived_at IS NULL`;
		// Drop cached authorization so the archived user's sessions stop resolving.
		invalidate_authz(id);
...
export async function restore_record(id: number): Promise<boolean> {
	try {
		const result = await db`UPDATE users SET archived_at = NULL, archived_by_user_id = NULL WHERE id = ${id} AND archived_at IS NOT NULL`;
		return (result.affectedRows ?? result.count ?? result.changes ?? 0) > 0;
	} catch (error) {
...
export async function get_archive_counts(scope_clause: string = ""): Promise<ArchiveCounts> {
	try {
		const counts_query = archive_counts_query("users", scope_clause);
