...
import { empty_strings, feature_paths, pick_body_fields, redirect_from_referer, run_bulk_remove } from "$lib/crud_routes";
...
export async function post_users_edit(req: BunRequest): Promise<Response> {
	...
	try {
		...
		sql_log({ s: "Update", t: `${feature}`, r: strip_log_sensitive(record) }, ctx.user?.username);
		await cache.invalidate(TABLE_NAME);
	} catch (error) {
...
export async function post_users_bulk_archive(req: BunRequest): Promise<Response> {
	const ctx = await create_ctx(req, import.meta.dir);
