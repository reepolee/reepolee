...
export function build_translation_breadcrumb_items(
	translations_label: string,
	base_path: string,
	namespace: string,
	parent: string,
): BreadcrumbItem[] {
	const current_label = parent ? `${namespace}.${parent}` : namespace;
	return [
		{ label: translations_label, href: base_path },
		{ label: current_label },
	];
}
...
export function flatten_to_keys(rows: TranslationRow[]): KeyRow[] {
	...
	for (const r of real) {
		...
		value_map.get(k)![r.locale] = r.translation;
	}
...
export function group_translations(rows: TranslationRow[]): GroupInfo[] {
	...
	const group_map = new Map<string, TranslationRow[]>();
	for (const r of real) {
