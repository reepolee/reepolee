...
export async function post_translations_add_namespace(req: BunRequest): Promise<Response> {
	...
	if (selected_templates.length > 0) {
		// Insert all template keys for all locales
		for (const tpl_id of selected_templates) {
			...
			for (const [key_path, en_value] of Object.entries(tpl.keys)) {
				...
				for (const locale of locales) {
					await upsert_translation(locale, namespace, full_key, en_value);
				}
			}
		}
	} else {
		// Insert a placeholder so the namespace appears
		await upsert_translation(default_locale, namespace, "_placeholder", "");
	}
...
export async function post_translations_add_group(req: BunRequest): Promise<Response> {
	...
	for (const locale of locales) {
		const value = params.get(`value__${locale}`)?.trim() || "";
		if (value) { await upsert_translation(locale, namespace, full_key, value); }
	}
...
export async function post_translations_bulk_delete(req: BunRequest): Promise<Response> {
	...
	try {
		...
		const toast_cookie = make_toast("toast-bulk-remove", { message, type: "green", duration: 4000 });
...
export async function post_translations_inline_save(req: BunRequest): Promise<Response> {
	try {
		...
		if (!namespace || !key_path || !locale) {
			return Response.json({ error: "Missing required fields: namespace, key_path, locale" }, { status: 400 });
		}

		if (value) {
			await upsert_translation(locale, namespace, key_path, value);
		} else {
			await delete_translation(locale, namespace, key_path);
		}
		...
		return Response.json({ ok: true, namespace, key_path, locale, value }, { status: 200 });
	} catch (err) {
