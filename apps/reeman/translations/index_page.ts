 ...
 * ree-filters multi-select), search, in-memory pagination over distinct
...
import { list_shadowed_translation_files } from "$lib/translation_files";
...
const SORT_OPTIONS = [
	{ value: "namespace::asc", field: "namespace", direction: "asc" },
	{ value: "namespace::desc", field: "namespace", direction: "desc" },
	{ value: "parent_path::asc", field: "parent_path", direction: "asc" },
	{ value: "parent_path::desc", field: "parent_path", direction: "desc" },
];
...
export async function get_translations_index(req: BunRequest): Promise<Response> {
	...
	// Read fresh from config/supported_locales.ts on every request instead of a
	// static top-level import - this module runs inside the reeman process,
	// which is never restarted after a locale is added/removed via the UI (only
	// --app is, via notify_server_reload()'s routes.ts touch), so a cached
	// import would keep showing the locale set from process startup.
	const { locales } = read_supported_locales();
	...
	// Locale files shadowed by a higher-priority app tree. Surfaced so users
	// can see that a file they know about is ignored and why, instead of only
	// seeing the console.warn during scanning.
	const shadowed_files = await list_shadowed_translation_files();

	// Extract unique namespace names for the Add Group dialog
	const namespaces = [...new Set(ns_groups.map((g) => g.namespace))].sort();

	// Count distinct keys and load the current in-memory page
	...
	const sort_dir = sort_parts[1]?.toLowerCase() === "desc" ? "desc" : "asc";
	...
	// Rows are sorted by (namespace|key_path, dir); re-sort using
	// the derived parent_path only to break ties within the same page for the
	// parent_path column (key_path prefix != parent_path in sort order for
	// keys with differing depths).
	const sort_multiplier = sort_dir === "desc" ? -1 : 1;
	flat_keys.sort((a, b) => {
		let cmp: number;
	...
	const grid_cols = `10ch auto auto auto ${locales.map(() => "1fr").join(" ")}`;
