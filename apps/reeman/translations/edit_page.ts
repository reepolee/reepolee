/**
 * Translations admin - edit form (per-key values across all locales).
...
import { build_translation_breadcrumb_items, group_translations } from "./helpers";
...
export async function get_translations_edit(req: BunRequest): Promise<Response> {
	...
	const breadcrumb_items = build_translation_breadcrumb_items(ctx.translations.nav.translations, base_path(), namespace, parent);
...
export async function post_translations_edit(req: BunRequest): Promise<Response> {
	...
	for (const [key, value] of params) {
		if (key.startsWith("value__")) {
			// Format: value__{locale}__{encoded_key_path}
			const rest = key.slice("value__".length);
			const delim = rest.indexOf("__");
			if (delim < 0) continue;
			const locale = rest.slice(0, delim);
			const encoded_key = rest.slice(delim + 2);
			const actual_key_path = decodeURIComponent(encoded_key);
			const val = value?.trim() || "";
			if (val) {
				promises.push(upsert_translation(locale, namespace, actual_key_path, val));
			} else {
				// Delete empty
				promises.push(delete_translation(locale, namespace, actual_key_path));
			}
		}
	}
	...
	if (new_key_name) {
		...
		for (const locale of locales) {
			const value = params.get(`_new_key__value__${locale}`)?.trim() || "";
			if (value) { promises.push(upsert_translation(locale, namespace, full_key, value)); }
		}
	}
