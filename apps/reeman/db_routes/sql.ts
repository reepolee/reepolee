...
import { refresh_db_routes, type DbRouteSnapshot } from "./sql.custom";

export const TABLE_NAME = "db_routes";
export const VIEW_DEPENDENCIES: string[] = [];

export interface Record extends DbRouteSnapshot {}
...
export async function search_records(search = "", offset = 0, limit = 20, order_by = "id::asc", _scope_clause = "", _filter_clauses: { clause: string; params: any[] }[] = []): Promise<{ records: Record[]; total: number; }> {
	try {
		return await timed_query(TABLE_NAME, "search_records", async () => {
			return search_metadata(await records(), search, offset, limit, order_by, ["url", "table_name", "module", "display"], _filter_clauses);
		});
	} catch (error) { console.error("Error searching records:", error); return { records: [], total: 0 }; }
}

export async function create_record(_record: Omit<Record, "id" | "display">): Promise<Record> { throw new Error("db_routes is a read-only metadata resource"); }
export async function update_record(_id: number, _record: Omit<Record, "id" | "display">): Promise<Record | undefined> { throw new Error("db_routes is a read-only metadata resource"); }
export async function delete_record(_id: number): Promise<boolean> { throw new Error("db_routes is a read-only metadata resource"); }
